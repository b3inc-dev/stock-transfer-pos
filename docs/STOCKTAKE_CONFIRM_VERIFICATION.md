...
- **ステータス変更（groupItems / status のメタフィールド）**  
  ...
  - write が失敗すると、`next.catch` で `formatSaveError(e)` のトーストが後から出る（2020–2021, 2110–2112, 2166–2167, 2286–2288）。  
  - この場合、ユーザーは一度「棚卸を完了しました」を見たあと、「保存に失敗しました」系のトーストが続く。  
  - **つまり「成功トーストは出たが、メタフィールドのステータス・groupItems だけがサーバーに保存されていない」状態は、バックグラウンド write 失敗時にあり得る。**  
