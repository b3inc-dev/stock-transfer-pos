...
| 対策 | 内容 | 実装 |
|------|------|------|
| **API 側で「見つからないときは短時間待って再検索」** | admin_webhook が検索で 0 件のとき、2.5 秒 sleep してから同じ条件で再検索を最大 12 回繰り返し（最大 30 秒）。ヒットした時点でループを抜けるため、早く届けば短い応答で返る。 | ✅ 共通モジュール `admin-webhook-retry.ts`（2.5秒×12回＝最大30秒）。`api.log-inventory-change`・`webhooks.orders.updated`・`webhooks.refunds.create` の全経路で適用。 |
