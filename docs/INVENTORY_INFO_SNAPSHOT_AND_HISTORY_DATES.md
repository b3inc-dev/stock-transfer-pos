...
### 管理画面を閉じたあと Cron で「Invalid API key or access token」になる理由（expiring offline token）

このアプリでは **有効期限付きオフラインアクセストークン**（`expiringOfflineAccessTokens: true`）を利用しています。オフラインアクセストークンは **約1時間で期限切れ**になり、**リフレッシュトークン**で更新する必要があります。

- **管理画面を開いているあいだ**: アプリや Shopify の認証フローがトークンを更新するため、問題なく動作します。
- **管理画面を閉じたあと**: 誰もトークンを更新しないため、**1時間ほど経つとアクセストークンが期限切れ**になります。
- **Cron がその後に実行されると**: 期限切れのトークンで Shopify API を呼ぶため、`[API] Invalid API key or access token (unrecognized login or wrong password)` が返ります。

**対応（コード側）**: `/api/inventory-snapshot-daily` では、Cron 実行時に「オフラインセッションのアクセストークンが期限切れ（またはまもなく期限切れ）なら、リフレッシュトークンで更新してから在庫スナップショットを保存する」処理を入れています。これにより、**管理画面を閉じていても、リフレッシュトークンが有効なかぎり（90日間）Cron は成功**します。

**運用**: リフレッシュトークンも **90日間使わないと失効**します。その場合は「そのショップの管理画面でアプリを1回開いてください」と案内してください。
