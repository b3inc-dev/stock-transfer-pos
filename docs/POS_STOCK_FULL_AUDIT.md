...
| **高** | `any` の多用 | 型定義を追加し、`as any` を減らす。特に webhooks と apply-change 周り。 | ✅ 対応済み（app 配下の TypeScript で `any` を全削除。app/types/graphql-responses.ts・InventoryCountLike/InventoryCountEntryLike/InputLikeEvent 等を追加し、全 routes・utils・webhooks で型を導入。extensions は JavaScript のため対象外） |
