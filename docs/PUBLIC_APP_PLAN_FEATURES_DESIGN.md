...
- **開発ストアの判定**：GraphQL Admin API の `shop { plan { partnerDevelopment } }` で開発ストアかどうかを取得する。`partnerDevelopment === true` のときは**課金を作成・請求せず**、アプリ内では `plan = "pro"`（全機能利用可能）として扱う。
...
| **課金** | 公開アプリのみ。3/10 loc は Managed pricing（Recurring）。10 loc 超は **Usage-based billing** で「基本料金＋(ロケーション数−10)×単価」（要件確定）。開発ストアは請求しない。 |
...
| 3〜 | 在庫情報・入出庫・仕入・ロス・発注・棚卸・調整 | 従来どおり | プランに応じて表示／非表示（4.2 の機能フラグ） |
| 最後 | **料金プラン** | `/app/plan` | Lite / Pro の違いの説明、プラン選択・アップグレードへの導線（公開アプリ時のみ、ナビの一番下） |
