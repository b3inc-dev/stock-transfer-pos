...
export function InventoryCountProductGroupSelection({
  ...
  useEffect(() => {
    if (!count) return;
    const productGroupIds = Array.isArray(count.productGroupIds) ? count.productGroupIds : [];
    setProductGroups(productGroupIds.map((id) => ({ id, name: productGroupNames.get(id) || id })));
      ...
      await Promise.all(
        productGroupIds.map(async (groupId) => {
          try {
            ...
            qtyMap.set(groupId, { total: totalQty, actual: actualQty, status });
          } catch (e) {
            console.error(`Failed to get quantity for product group ${groupId}:`, e);
            qtyMap.set(groupId, { total: 0, actual: 0, status: "未処理" });
          }
        })
      );
    } catch (e) {
          ...
          {count ? (
            <s-stack gap="none">
              <s-text tone="subdued" size="small">
                棚卸ID: {String(count?.countName || count?.id || "").trim() || "棚卸ID"}
              </s-text>
              <s-text tone="subdued" size="small">
    ...
    <s-box padding="base">
      <s-stack gap="none">
        {productGroups.map((group, index) => {
          ...
          const qtyInfo = productGroupQuantities.get(groupId) || { total: 0, actual: 0, status: "未処理" };
