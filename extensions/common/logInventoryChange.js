 ...
 * Phase0: appEventId で1操作を一意に束ね、失敗チャンクは握りつぶさず throw、lineItems は Map で O(1) 参照。
 *
 * @param {Object} opts
 * @param {string} [opts.appEventId] - 1回の操作単位の一意ID（省略時は内部で生成）。再送・突合・idempotency の軸
...
export async function logInventoryChangeToApi({
  ...
  const appEventId = typeof appEventIdIn === "string" && appEventIdIn.trim() ? appEventIdIn.trim() : (typeof crypto !== "undefined" && crypto.randomUUID ? crypto.randomUUID() : `evt_${Date.now()}_${Math.random().toString(36).slice(2, 10)}`);
  ...
  if (!token) {
    throw new Error("logInventoryChangeToApi: Failed to get session token");
  }
  ...
  const lineItemMap = new Map(
    (lineItems || []).map((l) => [String(l?.inventoryItemId ?? "").trim(), l]).filter(([k]) => k)
  );
  ...
  for (const d of deltas) {
    ...
    seq += 1;
    const li = lineItemMap.get(String(d.inventoryItemId).trim());
    const logData = {
      appEventId,
      entrySeq: seq,
