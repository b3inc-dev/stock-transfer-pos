...
// 発注用の下書きキー（ロスとは別namespace）
const ORDER_DRAFT_KEY = "stock_transfer_pos_order_draft_v1";
const ORDER_CONDITIONS_DRAFT_KEY = "stock_transfer_pos_order_conditions_draft_v1";
const SCAN_QUEUE_KEY = "stock_transfer_pos_order_scan_queue_v1";
...
export function OrderProductList({ conds, onBack, onAfterConfirm, setHeader, setFooter, liteMode: liteModeProp, onToggleLiteMode }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const saved = await SHOPIFY.storage.get(ORDER_DRAFT_KEY);
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      try {
        ...
        await SHOPIFY.storage.set(ORDER_DRAFT_KEY, {
          version: 1,
          savedAt: Date.now(),
          conds: {
  ...
  const handleConfirm = useCallback(async () => {
    ...
    try {
      ...
      const existingRaw = await readOrderEntries();
      ...
      const entry = {
        ...
        locationId: conds.locationId,
        locationName: conds.locationName ?? "",
        destination: conds.destination || null,
        date: conds.date ?? "",
        desiredDeliveryDate: conds.desiredDeliveryDate || null,
        note: conds.note ?? "",
        staffName: conds.staffName ?? "",
        items,
        status: "pending",
      ...
      try {
        if (SHOPIFY?.storage?.delete) {
          await SHOPIFY.storage.delete(ORDER_DRAFT_KEY);
          await SHOPIFY.storage.delete(ORDER_CONDITIONS_DRAFT_KEY);
        }
      } catch (e) {
  ...
  }, [setHeader, conds?.locationName, conds?.locationId, conds?.date, conds?.staffName, query, candidates.length, loading, searchMountKey, liteMode, handleToggleLiteMode]);
  ...
  useEffect(() => {
    ...
    setFooter(
      <FixedFooterNavBar
        ...
        rightCommand="--show"
        rightCommandFor={CONFIRM_ORDER_MODAL_ID}
        rightTone="success"
  ...
  const orderAddedLineRowMemoRef = useRef(null);
  if (orderAddedLineRowMemoRef.current === null) {
    orderAddedLineRowMemoRef.current = memo(function OrderAddedLineRowMemo({ line, inc, setQty, remove }) {
      return (
        <LossAddedLineRow
          line={line}
          onDec={() => inc(line.id, -1)}
  ...
  const OrderAddedLineRowMemo = orderAddedLineRowMemoRef.current;
  ...
  return (
    <s-stack gap="none">
      ...
      {/* 確定確認モーダル（発注リスト保存） */}
      <s-modal id={CONFIRM_ORDER_MODAL_ID} heading="発注リストを保存しますか？" ref={confirmLossModalRef}>
        <s-box padding="base" paddingBlockEnd="none">
          <s-stack gap="extra-tight">
          ...
          <s-box>
            <s-button
              command="--hide"
              commandFor={CONFIRM_ORDER_MODAL_ID}
              onClick={() => {
                // 何も実行せずにモーダルを閉じる
              }}
            >
