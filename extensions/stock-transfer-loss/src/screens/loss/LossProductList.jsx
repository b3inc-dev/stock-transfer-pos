...
export function LossProductList({ conds, onBack, onAfterConfirm, setHeader, setFooter, liteMode: liteModeProp, onToggleLiteMode }) {
  ...
  const handleLoadMoreSearch = useCallback(async () => {
    ...
    try {
      ...
      setSearchPageInfo(pageInfo);
      setCandidatesDisplayLimit((prev) => prev + list.length); // 追加分も表示
    } catch (e) {
  ...
  const lossAddedLineRowMemoRef = useRef(null);
  if (lossAddedLineRowMemoRef.current === null) {
    lossAddedLineRowMemoRef.current = memo(function LossAddedLineRowMemo({ line, inc, setQty, remove }) {
      return (
        <LossAddedLineRow
          line={line}
          onDec={() => inc(line.id, -1)}
  ...
  const LossAddedLineRowMemo = lossAddedLineRowMemoRef.current;
