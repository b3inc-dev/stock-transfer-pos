...
function Extension() {
  ...
  const [lossView, setLossView] = useState(LOSS_VIEW.CONDITIONS);
    ...
    try {
      ...
      unsub = scannerApi.subscribe((result) => {
        try {
          ...
          if (currentLossView === LOSS_VIEW.PRODUCT_LIST) {
            pushScanToQueue_(data);
          }
        } catch (e) {
          console.error("[Loss] scanner callback error:", e);
        }
      });
    } catch (e) {
      console.error("[Loss] scanner subscribe error:", e);
    }
    return () => {
      try { unsub?.(); } catch {}
    };
