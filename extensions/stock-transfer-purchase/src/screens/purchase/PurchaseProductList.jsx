...
export function PurchaseProductList({ conds, onBack, onAfterConfirm, setHeader, setFooter, liteMode: liteModeProp, onToggleLiteMode }) {
  ...
  const upsertLineByResolvedVariant = useCallback(
    (resolved, { incBy = 1, closeSearch = true, toastOnNew = true } = {}) => {
      ...
      const titleForToast =
        String(resolved.barcode || "").trim() ||
        String(resolved.sku || "").trim() ||
  ...
  const purchaseAddedLineRowMemoRef = useRef(null);
  if (purchaseAddedLineRowMemoRef.current === null) {
    purchaseAddedLineRowMemoRef.current = memo(function PurchaseAddedLineRowMemo({ line, inc, setQty, remove }) {
      return (
  ...
  const PurchaseAddedLineRowMemo = purchaseAddedLineRowMemoRef.current;
