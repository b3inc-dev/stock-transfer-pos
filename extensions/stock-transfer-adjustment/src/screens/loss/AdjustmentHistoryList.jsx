...
import { readAdjustmentEntries, writeAdjustmentEntries, adjustInventoryToActual, fetchLocations, fetchVariantImage } from "./adjustmentApi.js";
...
function formatAdjustmentName(entry, allEntries, index) {
...
export function AdjustmentHistoryList({ onBack, locations: locationsProp = [], setLocations, setHeader, setFooter, liteMode, onToggleLiteMode }) {
  ...
  const refreshLossHistory = useCallback(async () => {
    ...
    try {
      const list = await readAdjustmentEntries();
  ...
  const handleCancel = useCallback(
    async (entry) => {
      ...
      try {
        // キャンセル時は在庫を元の currentQuantity に戻す
        ...
        <s-box padding="base">
          <s-stack direction="inline" justifyContent="space-between" alignItems="center" gap="base">
            <s-box style={{ flex: "1 1 auto", minInlineSize: 0 }}>
              <s-stack gap="extra-tight">
                <s-text emphasis="bold">{adjName}</s-text>
  ...
  useEffect(() => {
    if (detailId) {
      ...
      const totalQty = (entry.items ?? []).reduce((s, it) => s + (Number(it.quantity) ?? 0), 0);
      setFooter?.(
        <FixedFooterNavBar
  ...
  if (detailId) {
    ...
    return (
      <>
        {/* ✅ キャンセル確認モーダル */}
        <s-modal id={CANCEL_CONFIRM_MODAL_ID} heading="調整をキャンセルしますか？">
        ...
        <s-box padding="base">
          <s-stack gap="base">
            ...
            // 履歴は編集不可。在庫/実数を表示（currentQuantityが無い場合は実数のみ）
            const curQ = it.currentQuantity != null ? Number(it.currentQuantity) : null;
            const actQ = Number(it.quantity ?? 0);
            const belowRight = curQ != null ? `在庫: ${curQ} / 実数: ${actQ}` : `実数: ${actQ}`;
