...
import { AdjustmentScreen } from "./screens/AdjustmentScreen.jsx";
...
const ADJUSTMENT_SCAN_QUEUE_KEY = "stock_transfer_pos_adjustment_scan_queue_v1";
...
async function pushScanToQueue_(value) {
  ...
  try {
    const now = Date.now();
    const cur = normalizeScanQueueObj_(await storage.get(ADJUSTMENT_SCAN_QUEUE_KEY));
    ...
    await storage.set(ADJUSTMENT_SCAN_QUEUE_KEY, {
      items: trimmed,
      lastV: v,
      lastT: now,
      updatedAt: now,
...
// AdjustmentScreen内のVIEW定数（AdjustmentScreen.jsxと同期）
const ADJUSTMENT_VIEW = { CONDITIONS: "conditions", PRODUCT_LIST: "productList", HISTORY: "history" };
...
function Extension() {
  ...
  const [adjustmentView, setAdjustmentView] = useState(ADJUSTMENT_VIEW.CONDITIONS);
  const liteMode = !!prefs?.liteMode;
  const onToggleLiteMode = useCallback(() => {
    setPrefs((p) => ({ ...(p && typeof p === "object" ? p : {}), liteMode: !p?.liteMode }));
  }, [setPrefs]);

  const adjustmentViewRef = useRef(adjustmentView);
  useEffect(() => {
    adjustmentViewRef.current = adjustmentView;
  }, [adjustmentView]);
    ...
    try {
      ...
      unsub = scannerApi.subscribe((result) => {
        try {
          ...
          if (!data) return;
          toast(`スキャン: ${data}`);
          const currentView = adjustmentViewRef.current;
          if (currentView === ADJUSTMENT_VIEW.PRODUCT_LIST) {
            pushScanToQueue_(data);
          }
        } catch (e) {
          console.error("[Adjustment] scanner callback error:", e);
        }
      });
    } catch (e) {
      console.error("[Adjustment] scanner subscribe error:", e);
    }
    return () => {
      try { unsub?.(); } catch {}
    };
  ...
  const body = (
    <AdjustmentScreen
      ...
      setHeader={setHeader}
      setFooter={setFooter}
      onViewChange={setAdjustmentView}
      liteMode={liteMode}
