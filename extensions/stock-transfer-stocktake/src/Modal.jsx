...
async function pushScanToQueue_(value) {
  ...
  try {
    const now = Date.now();
    const cur = normalizeScanQueueObj_(await storage.get(STOCKTAKE_SCAN_QUEUE_KEY));
    ...
    await storage.set(STOCKTAKE_SCAN_QUEUE_KEY, {
      items: trimmed,
      lastV: v,
      lastT: now,
      updatedAt: now,
...
function Extension() {
  ...
  const [stocktakeView, setStocktakeView] = useState(STOCKTAKE_VIEW.CONDITIONS);
    ...
    try {
      ...
      unsub = scannerApi.subscribe((result) => {
        try {
          ...
          if (!data) return;
          toast(`スキャン: ${data}`);
          if (stocktakeViewRef.current === STOCKTAKE_VIEW.PRODUCT_LIST) {
            pushScanToQueue_(data);
          }
        } catch (e) {
          console.error("[Stocktake] scanner callback error:", e);
        }
      });
    } catch (e) {
      console.error("[Stocktake] scanner subscribe error:", e);
    }
    return () => {
      try {
        unsub?.();
