...
function Extension() {
  ...
  // 追加：画像表示モード切替（showImagesは派生なので触らない）
  ...
  useEffect(() => {
    ...
    setFooter?.(
      <FixedFooterNavBar
        ...
        middleLabel={`画像表示:${liteMode ? "OFF" : "ON"}`}
...
function OutboundHistoryConditions({
  ...
  // Footer（戻る／画像表示／再取得）…InboundConditionsと同型
  useEffect(() => {
    const summaryLeft = `出庫元: ${originLocationName}`;
    const summaryRight =
      historyMode === "shipped"
    ...
    setFooter?.(
      <FixedFooterNavBar
        ...
        leftLabel="戻る"
        onLeft={onBack}
        middleLabel={liteMode ? "画像表示:OFF" : "画像表示:ON"}
        middleTone={liteMode ? "critical" : "default"}
        onMiddle={onToggleLiteMode}
        middleDisabled={typeof onToggleLiteMode !== "function"}
        rightLabel={historyLoading ? "読込中..." : "再取得"}
...
function OutboundList({
  ...
  const originLocationGid =
    String(appState?.originLocationIdManual || "").trim() || useOriginLocationGid() || null;
...
function InboundConditions({
  ...
  const locationGid = String(appState?.originLocationIdManual || "").trim() || useOriginLocationGid() || null;
  const locIndex = useLocationsIndex(appState, setAppState);
...
function InboundList({
    ...
    try {
      ...
      if (!isMultipleMode && typeof onAfterReceive === "function") {
        ...
        if (transferId) {
          ...
          onAfterReceive(transferId).catch(() => {});
          return true;
        }
      }
  ...
  const renderExtrasHistory_ = () => {
    if (extrasHistoryLoading) {
      return <s-text tone="subdued" size="small">読み込み中...</s-text>;
    }
