...
**最終更新日**: 2026年3月5日（**POS棚卸：単一商品グループで商品リストが読み込まない不具合を修正**：Conditions でタップ時に productGroupIds が無い場合は productGroupId から配列を生成（単一・まとめて表示の両方）。InventoryCountList の loadProducts で fullCount に locationId が無い場合は count をフォールバックし、loadProducts 内で locationId を統一使用。ProductGroupSelection で productGroupIds 解決を resolvedProductGroupIds に統一し productGroupId フォールバックを追加。複数グループ経由・まとめて表示・単一タップの3経路で商品リスト読み込みロジックを同一化。実装: InventoryCountConditions.jsx, InventoryCountList.jsx, InventoryCountProductGroupSelection.jsx。） 2026年3月4日（**2026-03 実装情報追記**：確定処理（差異あり/なし・商品グループごと・まとめて・単一の3表示モード）で問題なく処理されることを検証・docs/STOCKTAKE_CONFIRM_VERIFICATION.md に追記。確定ボタン押下後は成功するまで自動保存（下書き）が消えない（4経路とも clearAllInventoryCountDraftsForCount は API 成功時のみ実行）。管理画面で棚卸ID発行時の過去ID名称との重複は通常なし（getNextCountNumber＋appendNewCountToChunked、追加後に NEXT_KEY 更新）。同時発行時のみ同一 countName の重複リスク。repair_count_names 等では同一名称・別 id を検出して保存拒否。mergeExistingNonBlank で既存 completed/cancelled を in_progress で上書きしないステータスダウングレード防止。空でのメタ上書き防止（メインキー確認失敗時も throw でブロック）。履歴表示で list が空かつ main がチャンクのとき main チャンクから一覧取得するフォールバック。POS：単一グループ・まとめて表示の読み込みを商品グループごと表示と同様に最適化（並列取得）。管理画面：未完了グループの商品リストを3 fetcher 並列＋キューで取得。メタフィールド復元UIは SHOW_METAFIELD_RESTORE_UI=false で非表示（コードは残置）。docs: STOCKTAKE_CONFIRM_VERIFICATION.md, METAFIELD_UPDATE_ISSUES.md, STOCKTAKE_COUNTNAME_AUDIT.md, STOCKTAKE_POS_LOAD_SPEED.md。） 2026年2月28日（**棚卸確定まわり（追補）**：単一グループでステータスが「完了」にならない不具合を修正（buildUpdatedCountFromLocalState で resolvedAllIds・mergeCountWithStorage で groupIdsForCheck。在庫調整で Throttled 時もリトライ（adjustInventoryToActual に runWithThrottleRetry と throttle/429 を追加）。確定後のメタ read/write に runWithBackgroundWriteRetry（最大3回・2秒×試行）を4経路すべてに適用。writeInventoryCounts で既存より少ない件数で上書きしないガード。単一グループ下書きは count 単位キーで保存・復元・削除（LEGACY フォールバック）。確定時 fromStorage ?? count と read リトライで他グループの groupItems を落とさない。検証: docs/STOCKTAKE_CONFIRM_VERIFICATION.md。）**POS棚卸確定処理の待機時間最小化・下書き削除タイミング・直列化**：差異あり・差異なしの全4経路で「UIを先に完了（toast/onAfterConfirm/setSubmitting(false)）→ read/write/clear はバックグラウンド」に統一しアプリタイル上の待機時間を最小化。自動保存（下書き）は write 成功時のみ clearAllInventoryCountDraftsForCount で削除し、確定失敗まで消えないように統一。同一棚卸で連続確定した場合の競合防止のため pendingBackgroundWriteByCountId で count 単位にバックグラウンド write を直列化。実装: InventoryCountList.jsx。**管理画面ソート**：棚卸・入出庫・仕入・発注・ロス・調整の各一覧でフィルター下にソートカードを追加。棚卸は棚卸ID昇順/降順（デフォルト降順）、入出庫は日付昇順/降順、仕入・発注・ロス・調整は各ID昇順/降順（いずれもデフォルト降順）。「棚卸ID順に並び替え」ボタンは削除。実装: app.inventory-count.tsx, app.history.tsx, app.purchase.tsx, app.order.tsx, app.loss.tsx, app.adjustment.tsx。**棚卸ID番号重複防止**：管理画面の新規棚卸発行時、create_inventory_count で保存直前に readInventoryCountsChunked で最新一覧を取得し ensureCountNamesOnCounts で付与するよう変更（同時発行での重複を防止。docs/STOCKTAKE_COUNTNAME_AUDIT.md に「番号重複の防止」追記）。**POS棚卸**：保存失敗時の Throttled 対策として runWithThrottleRetry（最大2回・2秒遅延）・バッチ間 150ms 遅延を writeInventoryCounts に追加。ユーザー向けメッセージを「サーバー負荷により一時制限されています。しばらく待ってから再度「確定」を押してください。」に変更（stocktakeApi.js, InventoryCountList.jsx の formatSaveError）。**棚卸**：「商品グループをすべて完了にする」ボタンを非表示に変更。**POS棚卸アプリタイル一覧**：作成日（createdAt）降順で表示（InventoryCountConditions.jsx）。）**在庫情報：useMemo is not defined 対策（React を import し useMemo を React.useMemo に変更。本番バンドルでエラー解消）。** **棚卸：棚卸IDが空白で上書きされないよう write 前に ensureCountNamesBeforeWrite（stocktakeApi.js）・Conditions のバックグラウンドマージで full.countName を反映（InventoryCountConditions.jsx）。** **管理画面：棚卸IDを修復する機能を追加（発行タブに「棚卸IDを修復」ボタン。action repair_count_names で readInventoryCountsChunked→ensureCountNamesOnCounts→writeInventoryCountsChunked。draft が POS に表示されない場合でも空白の棚卸IDを一括修復可能。docs/STOCKTAKE_COUNTNAME_AUDIT.md 追加。** **棚卸：在庫差異なし時もUI先完了・確定後遷移でステータス即時反映（2026-02-28）。** 差異なし2経路も即時 toast・onAfterConfirm・setSubmitting(false) の後、read→write→clear を pendingBackgroundWriteByCountId で直列化したバックグラウンド実行。確定後遷移：StocktakeScreen の handleAfterConfirm で setCount のあと、status===completed ならコンディション画面、未完了ありなら商品グループリストへ遷移。遷移先で親 state の更新済み count を表示するため、モーダルを閉じるまでステータスが完了にならない事象を解消。実装: InventoryCountList.jsx, StocktakeScreen.jsx。** **棚卸：アプリタイルリスト・確定処理高速化・完了表示・merge安全化（2026-02-28）。** ①アプリタイルからリスト表示時に棚卸IDが消えない：StocktakeScreen で handleAfterConfirm(null) 時は何もしない。InventoryCountConditions で refresh 失敗時も setCounts([]) しない。②確定処理の高速化：InventoryCountList で buildUpdatedCountFromLocalState によりローカルで更新後 count を組み立て、readInventoryCountsRaw をブロックせず即「完了」表示。4経路すべてで read をバックグラウンドに移動。stocktakeApi の readInventoryCountsFirstPage で list/フルチャンクを Promise.all で並列取得。③完了が未処理に戻る不具合：fixCountsStatusOnly で status===completed は上書きしない。readInventoryCounts でも completed を in_progress に戻さない（stocktakeApi.js）。④確定後 merge・ID比較の安全化：read に当該 count が無い場合は配列に追加して write。ID は String 比較に統一。InventoryCountConditions のバックグラウンド詳細マージ・InventoryCountList/InventoryCountProductGroupSelection の draft 更新で ID を String 比較・allCounts を Array ガード（InventoryCountConditions.jsx, InventoryCountList.jsx, InventoryCountProductGroupSelection.jsx, stocktakeApi.js）。** **棚卸：在庫有効化エラー時メッセージを「在庫有効化に失敗しました」に統一、有効化の成功率向上（追跡ON失敗時も inventoryActivate を試行・リトライ4回・遅延1s/800ms・アイテム間150ms・リトライ時は数量なしで activate・「already has」等を既に有効として成功扱い・除外せず全件有効化するまでリトライ、失敗時は throw。stocktakeApi.js）。** **在庫変動履歴：棚卸のロケーションを名前で表示（locationName が GID のときも API・一覧・CSV で GraphQL または locations マップから解決）。** **棚卸：最後の1グループ確定時にステータスが「完了」になるよう修正（複数グループを1つずつ確定する場合、groupItems を親の count.groupItems とマージして allDone を判定。在庫調整あり・差分なしの両パスで対応。InventoryCountList.jsx）。** **棚卸番号名称（#C0001）をIDに固定・重複なく積み上げ（一度振った countName は再計算せず固定。欠けている場合のみ既存最大+1 から作成日時順で付与。管理画面の新規発行時は既存の最大番号+1 を付与。parseCountNameNumber で #C0001 形式から数値取得。stocktakeApi.js, app.inventory-count.tsx）。** 2026年2月27日分：**棚卸：在庫数＝実数で差分なし時の確定UX改善（確認モーダルで「確定する」で完了・先頭トースト削除・setSubmitting・モーダル文言2行「在庫数に差異はありません。」「確定する」で棚卸を完了できます。）。** **棚卸：ロケーションに在庫レベルがない時の確定失敗を解消（在庫有効化処理を先に実行してから確定可能に。stocktakeApi.js に ensureInventoryActivatedAtLocation を追加・adjustInventoryToActual の先頭で呼び出し）。** **棚卸：商品リスト250件以上で「さらに読み込み」が出ない可能性への対策（fetchProductsByGroups で初回 offset=0 かつ返却件数 >= effectiveFirst のとき hasMore を true に）。商品リスト読み込み中の「商品がありません」チラつき防止（loadCompletedRef で初回ロード完了後にのみ表示・1件以上読めたときだけ true）。棚卸ID採番を createdAt 昇順＋同順時 id で安定化（管理画面・POS とも同じ順で採番し #C0034 の後に作ったものが #C0028 になる事象を防止）。既に発行した誤った countName を次回読み込み時に作成順で再計算して修正（常に countName を再計算し変更時のみ保存）。** **棚卸：在庫数と実数が一致（差分なし）でも確定で完了できるようにする（まとめて表示で groupItems に保存して allDone で完了に。トーストは「在庫数に変更はありませんが、このまま確定して完了できます」・確定後は「棚卸を完了しました」のみに統一）。** **棚卸：まとめて表示の「null is not an object (evaluating 'Cn.map')」を解消（キャンセル済みで商品0件のとき completedItems が null で .map が落ちる不具合を (completedItems || []).map で防止。stocktakeApi の fetchProductsByGroups で groups を必ず配列に正規化）。** **棚卸：管理画面のCSVアップロード後の表示リストのカード枠・間隔を商品検索と統一（スクロール枠 padding 6px・カード間 marginTop 4px・listIndex で先頭のみ0・未選択時の borderBottom 削除）。カード下部の青線が切れないように表示枠内の余白を合わせた。** **棚卸：商品グループの表示を常に skus（CSV登録順）に統一（resolveSkusToInventoryItemIds の戻り順・管理画面/POSの表示ソート）。管理画面で並び順の説明を追加（商品検索：選択順、コレクション：コレクション選択順＋コレクション表示順、CSV：グループ名行順＋CSV行順）。商品検索のグループ名プレースホルダーを「例: 雑貨グループ」に変更。まとめて表示の「null is not an object (evaluating '_n.map')」を解消（fetchProductsByGroups 戻り値を loadProducts 内の全経路で配列正規化）。** **棚卸：一覧・商品グループ・管理画面履歴の「N件 N/N」をバックグラウンドで取得（先に描画し数値は後から流し込む）。POS棚卸一覧は readInventoryCountById を行ごと、POS商品グループ一覧は表示時に loadProductGroupQuantities を自動実行、管理画面履歴一覧は get_count_full をキューで1件ずつ。取得中は「…」表示。** **棚卸：まとめて表示で fetchProductsByGroups の戻り値を配列に正規化し D.map is not a function を解消（InventoryCountList.jsx の loadGroupProducts）。** **棚卸：管理画面の商品グループ編集でSKU件数が多いと商品リストが読み込めない不具合を修正（Shopify nodes(ids) は250件までなため getVariantsByInventoryItemIds/getVariantsBySkus で250件チャンクに分割。NODES_IDS_MAX=250）。** **POS：トーストをJAN（バーコード）優先で全タイル統一。連続スキャンをロス・発注・仕入・調整でも入庫・棚卸と同様にストレージ→メモリキュー→処理に統一。予定外棚卸不許可時トーストを「商品リストにない商品です」に変更。商品リスト行を memo 化（棚卸・入庫・出庫・ロス・発注・仕入・調整）し数量ボタン操作時の再描画を軽減。棚卸の在庫読み込みを getCurrentQuantitiesBulk（nodes クエリ・50件/リクエスト）で一括取得に変更し高速化。** **POS：ロス・発注・仕入・調整のスキャン処理を棚卸・入庫と同様に統一。追加時は即リスト反映＋トースト、在庫数はバックグラウンド取得。検索追加・数量ボタン・+-変更で既存行の数量だけ増える場合は在庫取得しない（新規行のみ在庫取得）。linesRef で新規行判定。** **棚卸：管理画面の棚卸ID発行を商品グループ「グループを追加」と同じUIに変更（白カード内見出し・青ボタン）。POS：商品グループ一覧で完了・未完了・キャンセルのステータスを count（groupItems/cancelledGroupIds）から即時表示、在庫数は読込ボタンで取得。** 2026-02-26：棚卸POS：fetchProductsByGroups で group.inventoryItemIds フォールバック（CSV由来でも商品リスト・在庫数読込が動作）。2026-02-25：棚卸：管理画面の商品リストモーダルで編集ボタン押下後も状態を維持（編集リセットはモーダル閉じ時のみ）。モーダルを閉じて再度開いたときに古い fetcher.data を無視（lastSubmittedForModalRef で今回の submit のレスポンスのみ処理）。同一ショップで複数ブラウザが重い棚卸モーダルを同時に開いた際の負荷軽減（get_incomplete_group_products のサーバー側キャッシュ・ショップ・棚卸・グループ・offset 単位・TTL2分・棚卸更新時無効化）。POS棚卸：まとめて表示・グループごとに表示で完了/キャンセル済みグループはAPIの groupItems を優先し未完了のみ下書きを使用（キャンセル・完了が正しく反映）。ドキュメント追加：ADMIN_INVENTORY_COUNT_MULTI_BROWSER_LOAD.md（複数ブラウザ負荷・同一ショップと複数ショップの違い・キャッシュ対策）・ADMIN_STOCKTAKE_MODAL_LOAD_AND_CONFIRM_CANCEL.md（商品リスト読み込み中に一括確定・一括キャンセルした場合の挙動）・STOCKTAKE_5SEC_SPEC_ESTIMATE.md（約5秒で処理完了する目安・グループ数が効くのはまとめて表示のみ）**。**棚卸：管理画面の完了/キャンセル時ボタン無効化・ステータス判定修正・モーダル読込の countId 無視・list 由来時 get_count_full 取得。全てキャンセル時の allCancelled を every() に変更。POS：キャンセルステータスとアプリタイル連動（readOnly に status=cancelled とキャンセル済みグループを追加・fixCountsStatusOnly で全グループキャンセル時は status=cancelled に補正・InventoryCountConditions の数量表示で cancelled 考慮）**。**棚卸：管理画面キャンセル対応でPOSアプリタイルに影響が出ないよう修正（cancelledGroupIds を完了判定に含める・status=cancelled は上書きしない・toMinimalCountForList に cancelledGroupIds/productGroupNames・POS の fixCountsStatusOnly/readInventoryCounts/InventoryCountList の allDone でキャンセル済みを完了とみなす）**。**棚卸：POS・管理画面の読み込み最適化（一覧用listメタフィールド・チャンク並列取得・リトライ・actionはcreate_inventory_count時のみ棚卸フル取得・モーダルlist-only時「商品リストを読込中...」表示・Renderタイムアウト注意）**。**棚卸：管理画面でCSVアップロードした商品グループの編集時に商品検索リストが表示されない不具合修正（getVariantsBySkus追加・SKUモード判定でskusありも対象）**。**棚卸・全タイル：追加読み込みタイムアウト延長（初回40秒・追加60秒）・読み込みボタン押下直後の「読込中...」即時表示（全タイル）**。**棚卸：商品リスト「さらに読み込む」250件超対応（コレクション経路 pageInfo/after）・下書き復元時の hasMoreProducts 再設定・まとめて表示の読込ボタン変数修正（c→count）・商品グループ一覧の在庫数読込のレート制限対策（グループ順次・15件バッチ）**。**ロス・注文・調整・発注：履歴詳細の商品リストで設定の初回表示件数と「さらに読み込む」を追加**。**アプリタイル一覧のロケーション・表示件数・タップ後詳細の統一（棚卸・仕入・発注・ロス・調整）**。2026年2月24日分：**アプリ表示件数統一・メタフィールド分割取得（発注・仕入・ロス・調整・棚卸）・設定画面の表示件数説明を最大・推奨のみに変更・棚卸POSのreadInventoryCountsFirstPage/PageとInventoryCountConditions分割取得対応**。**棚卸：商品リスト読み込み安定化・完了/処理中不整合解消・数量0/-対策・ステータス統一・グループ削除後完了扱い**。2026年2月22日分：**必須コンプライアンスWebhook・審査エラー検証通過**。上記に加え、**ホームスマホレイアウト・POS設定/販売チャネルリンク**・**設定画面の破棄・保存を下部固定フッター＋左にステータス表示**・**パートナー価格手順（PARTNER_PRICING_SETUP_STEPS.md）・4プラン（lite-3/lite-10/pro-3/pro-10）・ロケーション数とプラン不一致時の案内と機能制限・Pro料金$120(10loc)/$12(従量)** を反映。2026年2月21日分：公開アプリ課金実装・POS出庫確定後処理の他タイル統一・発注希望納品日UI 等）2026年2月20日分：管理画面・調整の改善（CSV差分・メモ表示・商品リストCSV設定連動）・在庫変動履歴の返品フィルター1つに統一、簡易棚卸（調整）機能・調整設定・変動履歴アクティビティ「調整」・商品リストフッター明細統一、入庫メモ1回追記・予定外表示フォールバック、POS出庫確定後コンディション遷移・下書きリセット修正、調整タイル・UID重複解消・order ??警告修正 等）
**コード確認日**: 2026年3月（**2026-03-05** POS棚卸：単一商品グループで商品リストが読み込まない不具合修正・3経路の商品リスト読み込み同一化を反映。） 2026年3月（**2026-03 実装情報追記**：確定・下書き・棚卸ID名称重複・ステータスダウングレード防止・空上書き防止・履歴フォールバック・POS/管理画面読み込み速度・メタ復元UI非表示を検証・REQUIREMENTS_FINAL.md と各 docs に反映。） 2026年2月（**2026-02-28 棚卸確定追補**：単一グループでステータス完了になるよう resolvedAllIds/groupIdsForCheck。adjustInventoryToActual に Throttled リトライ。確定後メタ read/write に runWithBackgroundWriteRetry（最大3回）。writeInventoryCounts で existing 不足分を merged に補完。単一グループ下書きは count 単位キー。fromStorage ?? count と read リトライ。docs/STOCKTAKE_CONFIRM_VERIFICATION.md。）**2026-02-28 POS棚卸確定**：全4経路でUI先完了→バックグラウンドwrite・下書きは確定成功時のみ削除・pendingBackgroundWriteByCountId で直列化。**2026-02-28 管理画面ソート**：棚卸・入出庫・仕入・発注・ロス・調整にフィルター下ソートカード（昇順/降順・デフォルト降順）。棚卸は「棚卸ID順に並び替え」ボタン削除。**棚卸ID番号重複防止**：create_inventory_count で保存直前に readInventoryCountsChunked→ensureCountNamesOnCounts。**POS棚卸 Throttled 対策**：runWithThrottleRetry・バッチ間遅延・formatSaveError でメッセージ改善。**棚卸**：「商品グループをすべて完了にする」非表示。**POS棚卸一覧**：createdAt 降順（InventoryCountConditions.jsx）。）**2026-02-28 在庫情報：useMemo を React.useMemo に変更（app.inventory-info.tsx）。棚卸：writeInventoryCounts 内で ensureCountNamesBeforeWrite、InventoryCountConditions で full.countName マージ、管理画面で repair_count_names・ensureCountNamesOnCounts・「棚卸IDを修復」ボタン（app.inventory-count.tsx）。docs/STOCKTAKE_COUNTNAME_AUDIT.md。** **2026-02-28 棚卸：在庫差異なし時もUI先完了・確定後遷移でステータス即時反映。** InventoryCountList：差異なしの2経路も即時 toast・onAfterConfirm・setSubmitting(false) の後、read→write→clear を pendingBackgroundWriteByCountId で直列化したバックグラウンド実行。StocktakeScreen：handleAfterConfirm で setCount のあと status===completed なら VIEW.CONDITIONS、未完了ありなら VIEW.PRODUCT_GROUP_SELECTION へ遷移。遷移先で更新済み count を表示するためモーダルを閉じるまでステータスが完了にならない事象を解消。** **2026-02-28 棚卸：アプリタイルリスト・確定処理高速化・完了表示・merge安全化。** StocktakeScreen：handleAfterConfirm(null) 時は count/view をクリアしない。InventoryCountConditions：refresh 失敗時も setCounts([]) しない。InventoryCountList：buildUpdatedCountFromLocalState でローカル組み立て・4経路で read をバックグラウンドに・確定後 merge で read に当該 count が無い場合は配列に追加・ID を String 比較・draft 更新で allCounts を Array ガード。stocktakeApi：fixCountsStatusOnly/readInventoryCounts で status===completed は上書きしない。readInventoryCountsFirstPage の list/フルチャンクを Promise.all で並列取得。InventoryCountConditions：バックグラウンド詳細マージで full.id/x.id を String 比較。InventoryCountProductGroupSelection：draft 更新で ID を String 比較・allCounts を Array ガード。** **2026-02-28 棚卸：在庫有効化エラー時「在庫有効化に失敗しました」に統一・有効化成功率向上（ensureInventoryActivatedAtLocation：追跡ON失敗時も activate 試行・リトライ4回・delayAfterTrackedMs 1000・アイテム間150ms・リトライ時は数量なしで activate・already has 等を既に有効として成功。adjustInventoryToActual：有効化失敗時は失敗アイテムのみ最大2回再試行・仍失敗時は除外せず throw「在庫有効化に失敗しました」。stocktakeApi.js）。** **2026-02-28 在庫変動履歴：棚卸のロケーションを名前表示（api.log-inventory-change で GID 時も resolveLocationName、app.inventory-info で locationNameByLocationId マップ・getChangeHistoryLocationDisplayName、export-change-history-csv.server で locations 取得・getLocationDisplayName。POS InventoryCountList で locationName を API に渡す）。** **2026-02-28 棚卸：最後の1グループ確定でステータス完了（InventoryCountList で groupItems を親の count.groupItems とマージ。在庫調整あり・差分なしの両パス）。** **2026-02-28 棚卸番号名称のID固定（stocktakeApi readInventoryCounts で既存 countName は変更せず欠けている場合のみ max+1 から付与。app.inventory-count で新規発行時に parseCountNameNumber で最大番号取得し max+1 付与）。** **2026-02-27 棚卸：在庫数＝実数で差分なし時の確定UX（先頭トースト削除・確定するで完了・モーダル文言2行化・setSubmitting。InventoryCountList.jsx）。** **2026-02-27 棚卸：ロケーションに在庫レベルがない時の確定失敗を解消（ensureInventoryActivatedAtLocation で在庫有効化・adjustInventoryToActual の先頭で実行。stocktakeApi.js）。** **2026-02-26 棚卸：さらに読み込み・商品がありませんチラつき・棚卸ID採番（stocktakeApi：hasMore 初回 effectiveFirst 以上で true、fixCountsStatusOnly で createdAt+id ソート・countName 常時再計算・anyCountNameChanged で保存。app.inventory-count：create 時も同順で採番。InventoryCountList：loadCompletedRef で「商品がありません」はロード完了後・1件以上読めたときのみ表示）。** **2026-02-26 棚卸：在庫＝実数で差分なしでも確定で完了（InventoryCountList：itemsToAdjust.length === 0 時のまとめて表示で groupItems に全グループ保存・トースト文言変更・確定後トーストは「棚卸を完了しました」のみ）。** **2026-02-26 棚卸：まとめて表示の null.map 追加対策（InventoryCountList：completedItems が null のとき (completedItems || []).map で防止。stocktakeApi：rawGroups を Array.isArray で正規化して groups で .filter が落ちないようにする）。** **2026-02-26 棚卸：CSVアップロード後の表示リストのカード枠・間隔を商品検索と統一（app.inventory-count.tsx）。** **2026-02-26 棚卸：商品グループ表示を skus 順に統一・並び順説明追加・プレースホルダー「雑貨グループ」・まとめて表示の null.map 修正（InventoryCountList loadProducts 全経路で fetchProductsByGroups 戻り値を配列正規化）。** **2026-02-26 棚卸：一覧・商品グループ・管理画面履歴のN件N/Nバックグラウンド取得（InventoryCountConditions, InventoryCountProductGroupSelection, app.inventory-count の listRowDetailFetcher/listCountDetails）。まとめて表示の fetchProductsByGroups 戻り値配列正規化（InventoryCountList loadGroupProducts）。商品グループ編集の getVariantsByInventoryItemIds/getVariantsBySkus を nodes(ids) 250件チャンクに分割（app.inventory-count.tsx）。** **2026-02-26 POS：トーストJAN優先・連続スキャン統一・予定外トースト文言・行メモ化・棚卸在庫一括取得。** **2026-02-26 POS：ロス・発注・仕入・調整のスキャン処理を棚卸・入庫と同様に統一。upsertLineByResolvedVariant で追加時は即 setLines＋トースト、fetchVariantAvailable はバックグラウンド。既存行の数量追加時は在庫取得しない（linesRef で isNewLine 判定、新規行のみ fetchVariantAvailable）。LossProductList.jsx, OrderProductList.jsx, PurchaseProductList.jsx, AdjustmentProductList.jsx。** 2026-02-26 棚卸：管理画面の棚卸ID発行を「グループを追加」と同じUIに変更。POS：商品グループ一覧でステータスを count から即時表示。fetchProductsByGroups の group.inventoryItemIds フォールバック。app.inventory-count.tsx, InventoryCountProductGroupSelection.jsx, stocktakeApi.js）
...
| 項目 | 状態 |
|------|------|
| 棚卸ID発行UI | ✅ 完了（2026-02-26。商品グループ設定の「グループを追加」と同じUIに変更：白カード内に「棚卸ID発行」見出し・s-divider、発行ボタンは青 #2563eb・角丸6px・無効時グレーで「グループを追加」と同一スタイル。実装: app.inventory-count.tsx） |
| 履歴モーダルのデバッグ情報削除 | ✅ 完了 |
...
| 棚卸まとめて表示の不具合修正 | ✅ 完了（0/0表示削除・2つ目以降の商品リスト読込・発行時全グループ分inventoryItemIds取得・502対策。2026-02-17にまとめて表示の安定化：readProductGroupsを1回取得してcachedProductGroupsで渡す・0件時1回リトライ・要因ドキュメント追記。要因・対策は docs/STOCKTAKE_MULTIPLE_GROUPS_PRODUCT_LIST_CAUSE.md） |
| 棚卸：CSVグループで履歴の数量母数・商品リスト表示 | ✅ 完了（2026-02-17。get_incomplete_group_products に skus フォールバック（パターン1b）を追加。要因・対策は docs/STOCKTAKE_CSV_GROUP_HISTORY_QUANTITY_CAUSE.md） |
| 棚卸：商品リスト読み込み安定化・完了/処理中不整合・数量0/-・ステータス統一・グループ削除後完了 | ✅ 完了（2026-02-24。管理画面：incompleteGroupProducts キー正規化・読み込み中表示・エラー時 loading 解除・locationId GID 正規化・一覧用 incompleteGroupProductsForList キー正規化・完了判定で削除済みグループは完了とみなす。POS：全て0・差分なし/在庫調整ありでも groupItems に保存・readInventoryCounts の後方互換廃止・削除済みグループは完了とみなす。要因・対策は docs/STOCKTAKE_ADMIN_VS_APP_STATUS_CAUSE.md） |
| アプリ表示件数の統一・メタフィールド分割取得・設定説明 | ✅ 完了（2026-02-24。アプリ表示件数を設定値で統一。発注・仕入・ロス・調整・棚卸のメタフィールドを first/after で分割取得。設定画面の表示件数項目説明を「最大・推奨」のみに変更。棚卸POS：stocktakeApi に readInventoryCountsFirstPage/Page とヘルパー追加、InventoryCountConditions を分割取得対応。実装: app.settings.tsx、各履歴loader、extensions/stock-transfer-stocktake） |
| アプリタイル一覧のロケーション・表示件数・タップ後詳細の統一（棚卸・仕入・発注・ロス・調整） | ✅ 完了（2026-02-25。①ロケーション：発注・仕入・ロス・調整でロケーションIDをGID/数値で正規化してフィルタ（他ロケーション非表示）。棚卸はフィルタ後0件でも全件フォールバックを削除。②並び順：全5タイルで履歴一覧を日付降順で表示。③タップ後詳細：発注・ロス・調整・仕入は一覧を軽く（itemsを落としたstate）、詳細はタップ時にrefから取得し入庫・出庫と同様の操作感に統一。実装: OrderHistoryList.jsx, LossHistoryList.jsx, AdjustmentHistoryList.jsx, PurchaseHistoryList.jsx, InventoryCountConditions.jsx。docs/APP_TILE_LIST_LOAD_AND_LOCATION_FIX.md） |
| 棚卸：商品リスト「さらに読み込む」250件超・下書き復元・まとめて表示読込・在庫数読込 | ✅ 完了（2026-02-25。①コレクション経路で pageInfo/after による次ページ取得を実装し、250件超も取得可能に（stocktakeApi.js の collectionPageInfo・InventoryCountList の collectionPageInfoRef）。②下書き復元時に hasMoreProducts を再設定し、未読込リストを復元後も読めるように。③まとめて表示の読込ボタンで c→count の参照ミスを修正。④商品グループ一覧の在庫数読込をグループ順次処理・getCurrentQuantity を15件バッチ化しレート制限で0件になる事象を防止。docs/STOCKTAKE_UX_TAP_AND_LOAD_BUTTONS.md） |
| 履歴詳細の商品リスト：初回表示件数・さらに読み込む（ロス・注文・調整・発注） | ✅ 完了（2026-02-25。各履歴一覧で詳細表示時の商品リストに設定の productList.initialLimit を反映し、超過分は「さらに読み込む」で表示。実装: LossHistoryList.jsx, OrderHistoryList.jsx, AdjustmentHistoryList.jsx, PurchaseHistoryList.jsx。docs/STOCKTAKE_UX_TAP_AND_LOAD_BUTTONS.md） |
| 棚卸・全タイル：追加読み込みタイムアウト延長・読み込みボタン即時フィードバック | ✅ 完了（2026-02-25。①棚卸 stocktakeApi：初回40秒・追加60秒に変更、fetchProductsByGroups内のgraphqlにtimeoutMsを渡す。②全タイルの読み込み系ボタン：setLoading(true)等の直後に await setTimeout(0) を挿入し、押した直後に「読込中...」を描画してからAPI取得開始。注文・ロス・調整・発注・入庫・出庫・棚卸の履歴再読込/読込/さらに読み込む/検索/在庫更新等。実装: stocktakeApi.js, 各 *HistoryList.jsx, *ProductList.jsx, ModalOutbound.jsx, InboundListScreen.jsx, Modal.jsx, InboundShipmentSelection.jsx, InventoryCountConditions.jsx, InventoryCountProductGroupSelection.jsx, OutboundReadyToShipEdit.jsx） |
| 棚卸：POS・管理画面の読み込み最適化（一覧list・並列・リトライ・action軽量化・list-onlyモーダル） | ✅ 完了（2026-02-25。POS: 一覧用メタフィールド inventory_counts_list_v1・棚卸ID→チャンクのインデックス・readInventoryCountsFirstPage/Page の並列化・軽量化・readInventoryCountById のインデックス参照。管理画面: loader で list 優先・フルチャンクは list が無い時のみ、チャンク並列取得・リトライ、action では create_inventory_count のときだけ readInventoryCountsChunked。モーダルで list 由来の棚卸時は「商品リストを読込中...」表示。Render タイムアウトは docs/STOCKTAKE_APP_TILE_LOAD_PERFORMANCE_ANALYSIS.md に記載） |
| 棚卸：管理画面でCSVアップロードした商品グループの編集で商品リストが表示されない不具合 | ✅ 完了（2026-02-25。編集時の SKU モード判定を「inventoryItemIds が空でも skus があれば SKU モード」に変更。getVariantsBySkus アクションを追加し、skus のみのグループ編集時に商品検索リストを表示。実装: app.inventory-count.tsx） |
| 棚卸POS：CSV・商品検索由来の商品グループで商品リスト・在庫数読込が空になる不具合 | ✅ 完了（2026-02-26。商品検索・CSVから作成をマストとするため、fetchProductsByGroups で棚卸の inventoryItemIdsByGroup が無い場合に product_groups_v1 の group.inventoryItemIds をフォールバック使用。CSV由来・コレクションなしのグループでも商品リスト表示・在庫数読込が動作。実装: extensions/stock-transfer-stocktake/src/screens/stocktake/stocktakeApi.js） |
| 棚卸：管理画面の棚卸ID発行UIをグループを追加と同じに | ✅ 完了（2026-02-26。白カード内見出し「棚卸ID発行」・発行ボタンを青ボタン（グループを追加と同じスタイル）に変更。実装: app.inventory-count.tsx） |
| 棚卸POS：商品グループ一覧でステータスを即時表示（在庫数は読込ボタン） | ✅ 完了（2026-02-26。完了・未完了・キャンセルを count の groupItems・cancelledGroupIds から即時表示。在庫数（件数・実数/在庫数）は従来どおり在庫数読込ボタンで取得。getCancelledGroupIdSet を stocktakeApi から export。実装: InventoryCountProductGroupSelection.jsx, stocktakeApi.js） |
| POS：ロス・発注・仕入・調整のスキャン処理統一・在庫取得は新規行のみ | ✅ 完了（2026-02-26。スキャン時は棚卸・入庫と同様に即リスト反映＋トースト、在庫数はバックグラウンド取得。検索追加・数量ボタン・+-変更で既存行の数量だけ増える場合は在庫取得しない（linesRef で isNewLine 判定、新規行のみ fetchVariantAvailable）。実装: LossProductList.jsx, OrderProductList.jsx, PurchaseProductList.jsx, AdjustmentProductList.jsx） |
| POS：トーストJAN優先・連続スキャン統一・予定外トースト・行メモ化・棚卸在庫一括取得 | ✅ 完了（2026-02-26。①トースト：追加時メッセージを全タイルで JAN（barcode）→ SKU → 商品名 の優先で統一。②連続スキャン：ロス・発注・仕入・調整でも入庫・棚卸と同様にストレージ→メモリキュー（scanQueueRef）→ 100ms tick で取り出し・processScanQueue で1件ずつ resolve/追加。③予定外棚卸不許可時：「商品リストにない商品です」に変更（設定に触れない文言）。④行メモ化：棚卸 InventoryCountLineRow・入庫 InboundAddedLineRowMemo・出庫 OutboundAddedLineRowMemo・ロス/発注/仕入/調整の各 AddedLineRowMemo を memo 化し、inc/setQty/remove 等を安定参照で渡して数量変更時は変更行のみ再描画。⑤棚卸在庫：getCurrentQuantitiesBulk（nodes クエリ・50件/リクエスト）を追加し、初回読込・追加読込・在庫更新・グループ一覧在庫・fetchProductsByGroups の在庫フィルタを一括取得に変更。実装: InventoryCountList.jsx, InboundUiParts.jsx, InboundListScreen.jsx, ModalOutbound.jsx, LossProductList.jsx, OrderProductList.jsx, PurchaseProductList.jsx, AdjustmentProductList.jsx, stocktakeApi.js, InventoryCountProductGroupSelection.jsx） |
| 棚卸：一覧・商品グループ・管理画面履歴の「N件 N/N」をバックグラウンドで取得 | ✅ 完了（2026-02-26。①POS棚卸一覧：表示後に readInventoryCountById を行ごとバックグラウンド実行し groupItems/items をマージ、取得中は「…」表示。②POS商品グループ一覧：表示時に loadProductGroupQuantities を自動実行し在庫数読込ボタンなしでN件 N/Nを後から表示。③管理画面履歴一覧：listRowDetailFetcher で get_count_full をキューで1件ずつ実行、listCountDetails にマージし取得中は「…」、行クリック時は取得済みデータをモーダルに渡して高速化。実装: InventoryCountConditions.jsx, InventoryCountProductGroupSelection.jsx, app.inventory-count.tsx） |
| 棚卸：まとめて表示の「D.map is not a function」修正 | ✅ 完了（2026-02-26。fetchProductsByGroups が { products, hasMore } を返す場合に .map で落ちる不具合。loadGroupProducts 内で戻り値を Array.isArray(x) ? x : (x?.products ?? []) で配列に正規化してから .map。実装: InventoryCountList.jsx） |
| 棚卸：まとめて表示の「null is not an object (evaluating '_n.map')」修正 | ✅ 完了（2026-02-26。fetchProductsByGroups が null 等を返した場合に .map/.forEach で落ちる不具合。loadProducts 内の単一グループ・まとめて表示の全経路で戻り値を Array.isArray(raw) ? raw : (raw?.products ?? []) で配列に正規化。実装: InventoryCountList.jsx） |
| 棚卸：まとめて表示の「null is not an object (evaluating 'Cn.map')」追加対策 | ✅ 完了（2026-02-26。要因：キャンセル済みで商品0件のグループで completedItems が null のまま「if (completedItems \|\| isGroupCancelled)」ブロックに入り、completedItems.map で落ちていた。対策：completedItems.map を (completedItems \|\| []).map に変更。あわせて stocktakeApi の fetchProductsByGroups で rawGroups を Array.isArray で正規化し groups の .filter が null で落ちないようにした。実装: InventoryCountList.jsx, stocktakeApi.js） |
| 棚卸：在庫数と実数が一致（差分なし）でも確定で完了できるようにする | ✅ 完了（2026-02-26。まとめて表示で itemsToAdjust.length === 0 のとき、hasCountedItems が false のグループも groupItems に保存するよう変更し allDone でステータスを完了に。トーストは「在庫数に変更はありませんが、このまま確定して完了できます」、確定後は「棚卸を完了しました」のみ（在庫調整なしの括弧を削除）。確定モーダル内の説明文も同趣旨に変更。実装: InventoryCountList.jsx） |
| 棚卸：商品リスト250件以上で「さらに読み込み」が出ない可能性への対策 | ✅ 完了（2026-02-26。fetchProductsByGroups で初回 offset=0 かつ uniqueVariants.length >= effectiveFirst のとき hasMore を true にし、設定上限以上あるのにボタンが表示されない事態を防止。実装: stocktakeApi.js） |
| 棚卸：商品リスト読み込み中の「商品がありません」チラつき防止 | ✅ 完了（2026-02-26。loadCompletedRef を追加し、初回ロード完了後にのみ「商品がありません」を表示。1件以上読めたとき・下書き復元・完了/キャンセル表示時のみ loadCompletedRef を true にし、コレクション→CSV の順で0件になる瞬間の表示を防止。実装: InventoryCountList.jsx） |
| 棚卸番号名称（#C0001）のID固定・重複なく積み上げ | ✅ 完了（2026-02-28。①一度振り分けた countName は同じ棚卸IDに固定（再計算・上書きしない）。②POS readInventoryCounts：既に countName がある場合はそのまま。欠けている場合のみ既存の最大番号+1 から作成日時順で付与し保存。③管理画面の新規発行時は既存の最大番号+1 を付与（sorted.length+1 を廃止）。parseCountNameNumber で #C0001 形式から数値を取得。実装: stocktakeApi.js, app.inventory-count.tsx） |
| 棚卸：ロケーションに在庫レベルがない時の確定失敗を解消・在庫有効化の成功率向上 | ✅ 完了（2026-02-27 初回、2026-02-28 強化。①在庫有効化：adjustInventoryToActual 実行前に ensureInventoryActivatedAtLocation で nodes(ids) により在庫レベル未作成のアイテムを検出し、inventoryItemUpdate(tracked:true)・inventoryActivate でロケーションに在庫レベルを作成してから inventorySetQuantities を実行。②2026-02-28：失敗時メッセージを「在庫有効化に失敗しました」に統一。有効化成功率向上のため、追跡ON失敗時も activate を試行、リトライ4回・delayAfterTrackedMs 1000・アイテム間150ms・リトライ時は数量なしで activate・「already has」等を既に有効として成功扱い。adjustInventoryToActual で有効化失敗時は失敗アイテムのみ最大2回再試行し、仍失敗時は除外せず throw。実装: extensions/stock-transfer-stocktake/src/screens/stocktake/stocktakeApi.js） |
| 棚卸：在庫数＝実数で差分なし時の確定UX改善 | ✅ 完了（2026-02-27。確認モーダルで「確定する」を押すと確定・完了できるように。先頭トースト「在庫数に変更はありませんが…」を削除し成功時は「棚卸を完了しました」のみ表示。itemsToAdjust.length===0 と allItemsToAdjust.length===0 の両パスで setSubmitting(true/false) を追加。モーダル文言を2行に変更「在庫数に差異はありません。」「確定する」で棚卸を完了できます。実装: InventoryCountList.jsx） |
| 棚卸：確定後のフッターステータス・差分あり/なしのバックグラウンド処理・待機時間最小化 | ✅ 完了（2026-02-27 初回、2026-02-28 拡張。①ステータス：全確定成功パスで onAfterConfirm(updatedCount) を渡し親で setCount してフッターを「完了」に即時更新。②**全4経路で待機時間最小化**：差異あり（まとめて・単一）は adjust 成功直後に toast・onAfterConfirm(locallyBuilt)・setSubmitting(false) でUIを先に完了し、read/write/clear はバックグラウンド。差異なし（まとめて・単一）も同様に即時 toast・onAfterConfirm(locallyBuilt)・setSubmitting(false) の後、read→write→clear をバックグラウンド実行。③**下書きは確定成功時のみ削除**：clearAllInventoryCountDraftsForCount は write 成功後の .then 内でのみ呼び出し。失敗時は呼ばず確定成功するまで自動保存は消えない。④**同一棚卸での連続確定時の競合防止**：pendingBackgroundWriteByCountId（Map）で count 単位にバックグラウンド write を直列化し、後者の read が前者の write 完了前に走って上書きする事象を防止。実装: InventoryCountList.jsx, StocktakeScreen.jsx） |
| 棚卸：確定まわり（単一グループステータス・在庫調整Throttled・メタread/writeリトライ・下書きキー・検証） | ✅ 完了（2026-02-28。①単一グループでステータスが「完了」にならない不具合：buildUpdatedCountFromLocalState で resolvedAllIds、mergeCountWithStorage で groupIdsForCheck。②在庫調整の Throttled 対応：adjustInventoryToActual 内で inventorySetQuantities を runWithThrottleRetry でラップ、リトライ条件に throttle/429 追加（stocktakeApi.js）。③確定後のメタ read/write リトライ：runWithBackgroundWriteRetry（最大3回・2秒×試行）を4経路すべてに適用。④writeInventoryCounts で既存より少ない件数で上書きしないガード（existing の不足分を merged に補完）。⑤単一グループの下書きは count 単位キー（inventoryCountDraftKeySingleGroup）で保存・復元・削除、復元時 LEGACY_KEY フォールバック。⑥確定時 fromStorage ?? count と read リトライで他グループの groupItems を落とさない。検証: docs/STOCKTAKE_CONFIRM_VERIFICATION.md、docs/STOCKTAKE_DRAFT_LOSS_CAUSES.md。実装: InventoryCountList.jsx, stocktakeApi.js） |
| 棚卸：最後の1グループ確定時にステータスが「完了」になるよう修正 | ✅ 完了（2026-02-28。複数グループを1つずつ確定する場合、確定時の groupItems を readInventoryCountsRaw の c.groupItems のみでなく親の count.groupItems とマージするよう変更。既に確定済みの他グループを含めて allDone を判定するため、最後の1つを確定した際にステータスが「完了」に更新される。在庫調整あり・差分なしの両パスで対応。実装: InventoryCountList.jsx） |
| 棚卸：在庫差異なし時もUI先完了・確定後遷移でステータス即時反映 | ✅ 完了（2026-02-28。①在庫差異なし：まとめて表示・単一グループの2経路とも、buildUpdatedCountFromLocalState で locallyBuilt を組み立てた直後に toast・onAfterConfirm(locallyBuilt)・setSubmitting(false) でUIを先に完了し、read→write→clear は pendingBackgroundWriteByCountId で直列化したバックグラウンド実行（setTimeout(0) 廃止・待機時間最小化）。②確定後遷移：StocktakeScreen の handleAfterConfirm で setCount のあと、status===completed ならコンディション画面、未完了ありなら商品グループリストへ遷移。実装: InventoryCountList.jsx, StocktakeScreen.jsx） |
| 棚卸：アプリタイルリスト・確定処理高速化・完了表示・merge安全化 | ✅ 完了（2026-02-28。①アプリタイルからリスト表示時に棚卸IDが消えない：handleAfterConfirm(null) 時は何もしない・refresh 失敗時も setCounts([]) しない。②確定処理の高速化：buildUpdatedCountFromLocalState でローカル組み立て・4経路すべて「UI先完了→read/write/clear はバックグラウンド」で待機時間最小化・下書きは write 成功時のみ clear・pendingBackgroundWriteByCountId で直列化。readInventoryCountsFirstPage のチャンクを Promise.all で並列取得。③完了が未処理に戻る不具合：fixCountsStatusOnly/readInventoryCounts で status===completed は上書きしない。④確定後 merge・ID 比較の安全化：read に当該 count が無い場合は配列に追加・ID を String 比較に統一・Conditions のバックグラウンド詳細マージ・draft 更新で allCounts を Array ガード。実装: StocktakeScreen.jsx, InventoryCountConditions.jsx, InventoryCountList.jsx, InventoryCountProductGroupSelection.jsx, stocktakeApi.js） |
| 棚卸：棚卸IDが空白で上書きされないよう write 前補完・表示時の countName 反映 | ✅ 完了（2026-02-28。①POS：writeInventoryCounts 内で ensureCountNamesBeforeWrite を実行し、確定後 readInventoryCountsRaw 由来の list を書くときも欠けている countName のみ付与。②POS：readInventoryCountById のバックグラウンドマージで full.countName を反映（InventoryCountConditions.jsx）。③監査メモ: docs/STOCKTAKE_COUNTNAME_AUDIT.md。実装: stocktakeApi.js, InventoryCountConditions.jsx） |
| 管理画面：棚卸IDを修復する機能 | ✅ 完了（2026-02-28。棚卸「発行」タブに「棚卸IDを修復」ボタン。action repair_count_names で readInventoryCountsChunked→ensureCountNamesOnCounts→writeInventoryCountsChunked。修復件数返却・成功時 revalidate で一覧即時反映。draft が POS に表示されない場合でも空白の棚卸IDを一括修復可能。実装: app.inventory-count.tsx） |
| 棚卸：空白のIDを絶対に作らない・空白で上書きしない・バックアップを残す | ✅ 完了（2026-02。①**空白のIDを生成しない**：書き込み直前に filterInvalidCountsBeforeWrite で id ありで countName または locationId が空白のレコードを配列から除外し保存しない。②**空白で上書きしない**：mergeExistingNonBlank で既存の locationId/productGroupIds/groupItems を補完（既存）。③**バックアップ**：書き込み前に現在の一覧（minimal）をメタフィールド inventory_counts_backup_v1 に 1 世代保存（60KB 以内）。POS: stocktakeApi.js。管理画面: app.inventory-count.tsx。監査: docs/STOCKTAKE_COUNTNAME_AUDIT.md） |
| 在庫情報：useMemo is not defined エラー対策 | ✅ 完了（2026-02-28。React を import し useMemo を React.useMemo に変更。loader エラー時はフォールバック返却・コンポーネントで loaderData.error 時は簡易エラーメッセージ表示。実装: app.inventory-info.tsx） |
| 棚卸：商品グループの表示を常に skus（CSV登録順）に統一 | ✅ 完了（2026-02-26。resolveSkusToInventoryItemIds の戻り値を入力 skus の並びで返すよう変更。getVariantsByInventoryItemIds/getVariantsBySkus/get_incomplete_group_products で返却を ids 順にソート。POS fetchProductsByGroups で inventoryItemIds 経由取得時は idsToFetch 順でソート。実装: app.inventory-count.tsx, stocktakeApi.js） |
| 棚卸：管理画面の商品グループ作成で並び順の説明を追加 | ✅ 完了（2026-02-26。商品検索・コレクション検索・CSVアップロードの各説明の下に「並び順：選択順」「並び順：コレクション選択順＋コレクション表示順」「並び順：グループ名行順＋CSV行順」を表示。商品検索のグループ名プレースホルダーを「例: 雑貨グループ」に変更。実装: app.inventory-count.tsx） |
| 棚卸：管理画面のCSVアップロード後の表示リストのカード枠・間隔を商品検索と統一 | ✅ 完了（2026-02-26。CSVプレビューリストのスクロール枠を商品検索と同じ padding: 6px に変更。flex/gap をやめ、カードは marginTop: 先頭0・2件目以降4px。listIndex を map の第2引数で取得するよう修正。未選択時の borderBottom を削除しカード枠を商品検索と同一に。カード下部の青線が切れないように表示枠内の余白を合わせた。実装: app.inventory-count.tsx） |
| 棚卸：管理画面の商品グループ編集でSKU件数が多いと商品リストが読み込めない不具合 | ✅ 完了（2026-02-26。要因：Shopify GraphQL nodes(ids) は250件まで。getVariantsByInventoryItemIds と getVariantsBySkus で全IDを1リクエスト送っていたため251件以上で失敗。対応：NODES_IDS_MAX=250 を定義し、ids を250件ずつチャンクに分割して nodes(ids) を複数回実行し結果をマージ。実装: app.inventory-count.tsx） |
| 棚卸：管理画面キャンセル対応でPOSアプリタイルに影響が出ないよう修正 | ✅ 完了（2026-02-25。管理画面でキャンセルしたグループをPOS側で正しく扱う。stocktakeApi: getCancelledGroupIdSet・fixCountsStatusOnly/readInventoryCounts で cancelledGroupIds を完了判定に含め status=cancelled は上書きしない・toMinimalCountForList に cancelledGroupIds/productGroupNames を追加。InventoryCountList: cancelledGroupIdSet・allDone でキャンセル済みグループを完了とみなす・newStatus で status=cancelled を保持。**2026-02-25 追加**：管理画面で完了/キャンセル時は編集・確定・キャンセル・リセットを全てグレーアウト、allCancelled を every() に変更、get_incomplete_group_products に countId で古いレスポンス無視、list 由来モーダルで get_count_full 取得。POS：readOnly に status=cancelled とキャンセル済みグループ（cancelledGroupIdSet）を追加、fixCountsStatusOnly で全グループキャンセル時は status=cancelled に補正、InventoryCountConditions の数量表示で cancelled 考慮。実装: app.inventory-count.tsx, stocktakeApi.js, InventoryCountList.jsx, InventoryCountConditions.jsx） |
| 棚卸：管理画面の商品リストモーダルで編集ボタン押下後すぐに状態が戻る不具合 | ✅ 完了（2026-02-25。要因：モーダル表示中の useEffect が modalCount?.id 等の変化で再実行され編集状態をリセットしていた。対応：編集モード・編集数量のリセットを「モーダルを閉じたとき」のみに限定。実装: app.inventory-count.tsx） |
| 棚卸：管理画面でモーダルを閉じて再度開くと読み込まれない商品グループが発生する不具合 | ✅ 完了（2026-02-25。要因：useFetcher の data はアンマウント後も残り、再オープン時に前回のレスポンスが処理されていた。対応：lastSubmittedForModalRef で「今回のモーダルで submit した countId/groupId」を保持し、fetcher.data はその組み合わせと一致するときのみ反映。実装: app.inventory-count.tsx） |
| 棚卸：同一ショップで複数ブラウザが重い棚卸モーダルを同時に開いた際の負荷軽減 | ✅ 完了（2026-02-25。get_incomplete_group_products のレスポンスをサーバー側で in-memory キャッシュ。キー: shop:countId:groupId:offset、TTL 2分。棚卸の数量更新・確定・キャンセル・リセット等の action で当該棚卸のキャッシュを無効化。docs/ADMIN_INVENTORY_COUNT_MULTI_BROWSER_LOAD.md） |
| 棚卸：POSアプリタイルでまとめて表示・グループごと表示時に完了/キャンセル済みグループが下書きで上書きされる不具合 | ✅ 完了（2026-02-25。完了・キャンセル済みグループは常に API の groupItems を表示し、未完了グループのみ下書きを参照。履歴から完了/キャンセル棚卸を開いた場合も下書き復元せず API データを表示。実装: InventoryCountList.jsx。docs/STOCKTAKE_DRAFT_AND_READONLY_QA.md） |
| 棚卸ページ上部メニュー下の余白 | ✅ 微調整済み（狭くした） |
| 設定の「破棄」「保存」ボタン | ✅ 完了（下部に固定フッターで常時表示。左にステータス「保存しました」「未保存の変更があります」等、右に破棄・保存ボタン。見逃し防止のためスクロール時も表示。実装: app.settings.tsx） |
...
| 入庫メモを1回追記に変更（在庫調整履歴の追記漏れ防止） | ✅ 完了（確定処理の最後に「予定外＋予定超過＋メモ＋在庫調整履歴」を1回で追記。2回目追記の失敗で在庫調整履歴が残らない事象を防止。InboundListScreen.jsx） |
| 入出庫履歴：予定外形式B(旧)のオプション表示フォールバック | ✅ 完了（タイトルに「 / 」が含まれる場合、商品名とオプションを分離して表示。過去データもオプション列に表示。app.history.tsx） |
...
| 在庫履歴のアクティビティ種別振り分け | ✅ 完了（管理/ロス/仕入/入出庫/棚卸/売上/返品/**キャンセル戻り（order_cancel）**/**調整（adjustment）**。Webhook二重防止・api/log-inventory-change で種別記録・上書き。2026-02-10 に出庫確定・入庫通常受領でのAPI呼び出しを追加し全確定処理で網羅。2026-02-13 にアクティビティ反映の強固化・二重防止・変動数「-」対策を追加。**2026-02-20** 簡易棚卸（調整）確定時に activity=adjustment で記録し、変動履歴のフィルター・ラベルに「調整」を追加。**2026-02-13 履歴UI**：仕入キャンセル→「仕入」、キャンセル戻り→「返品」に表示統一。変動後数量→変動後在庫数。getActivityDisplayLabel でラベル変換堅牢化。**フィルター**：仕入に続き「返品」を1つに統一（refund・order_cancel を選択時は両方検索）。実装: app.inventory-info.tsx） |
...
| 在庫変動履歴のロケーション名表示 | ✅ 完了（api/log-inventory-change でロケーション名が未設定・「出庫元」・**GID形式（gid://...）** のときも GraphQL で実ロケーション名を取得して保存。管理画面一覧では locations から GID/数値ID→名前のマップを組み、表示時に getChangeHistoryLocationDisplayName で解決（既存で DB に GID が入っている場合も名前表示）。CSV エクスポートでも同様に locations 取得してロケーション名を解決。POS 棚卸確定時は画面で取得済みの locationName を API に渡す。実装: api.log-inventory-change.tsx, app.inventory-info.tsx, app/export-change-history-csv.server.ts, InventoryCountList.jsx） |
...
| 簡易棚卸（調整）機能 | ✅ 完了（2026-02-20。POS拡張 stock-transfer-adjustment：ロスUIベース、コンディションはロケーション・日付・スタッフ・メモのみ。商品リストは在庫数＋実数、確定時は絶対数で差分調整・activity=adjustment で変動履歴に記録。管理画面：設定に「調整設定」タブ・調整履歴CSV出力項目設定、メニューに「調整」を棚卸の直下に配置。変動履歴のアクティビティに「調整」を追加。**管理画面改善**：調整設定CSV項目に「差分」を追加、調整履歴一覧・商品リストモーダルを「理由」→「メモ」表示に統一、商品リストモーダルのテーブルをCSV出力項目設定の並び・ON/OFFに連動。詳細は docs/SIMPLE_STOCKTAKE_ADJUSTMENT_FEASIBILITY.md） |
| 商品リスト固定フッターの明細表示統一 | ✅ 完了（2026-02-20。棚卸商品リストに「明細 N」を追加。入庫・出庫・仕入・発注・ロス・棚卸・調整の全商品リストで固定フッター中央に明細を含む表示に統一。2行以上のときは中央揃え。調整はスキャン・検索追加時は実数=1で追加し数量ボタンでカウント可能に） |
| 在庫変動履歴のアクティビティフィルター「返品」統一 | ✅ 完了（refund と order_cancel の2種を、仕入と同様にフィルター上は1つの「返品」に統一。選択時は両方を検索対象に展開。URL正規化で旧URLも対応。実装: app.inventory-info.tsx） |
| POS出庫：確定後のコンディション遷移・下書き削除の確実化（他タイルと同様の処理に統一） | ✅ 完了（2026-02-21。logInventoryChangeToApi を try/catch で囲み API 失敗時も下書き削除・遷移を実行。確定ボタン側でモーダル閉じたあとに onConfirmSuccess を実行。実装: ModalOutbound.jsx） |
| 発注：希望納品日の入力欄と日付ボタンの間隔・右寄せ（出庫の到着予定日と統一） | ✅ 完了（2026-02-21。gap=small・justifyContent=end・wrap。実装: OrderConditions.jsx） |
| ホームのスマホレイアウト | ✅ 完了（767px以下で現在の料金プラン・ロケーション数を最上部に100%幅で表示。PCでは従来どおり右カラム。order/flex で制御。実装: app._index.tsx） |
| ホームのPOSアプリタイル追加リンク | ✅ 完了（「POSの設定を開く」で販売チャネルPOSの設定を、「販売チャネル一覧を開く」でチャネル一覧を新しいタブで開く。storeHandle を loader/context で渡し、admin.shopify.com/store/{handle}/apps/point-of-sale-channel と settings/channels へリンク。実装: app._index.tsx, app.tsx） |
| 公開アプリ課金（プラン取得・サブスク作成・10loc超Usage・4プラン・ロケーション一致制限・Pro料金） | ✅ 完了（getShopPlan で currentAppInstallation.activeSubscriptions から plan 判定。app/utils/billing.ts：getPlanFromActiveSubscriptions・getMaxLocationsFromSubscriptionName・reportUsageRecord・createAppSubscription。**Pro料金**：10ロケーションまで $120/月、11ロケーション目以降 $12/ロケーション（BASE_PRICE_UP_TO_10.pro=120、USAGE_PRICE_PER_LOCATION.pro=12）。**4プラン対応**：パートナーで lite-3/lite-10/pro-3/pro-10 の Managed pricing を設定する手順は docs/PARTNER_PRICING_SETUP_STEPS.md。**ロケーション数とプラン不一致時**：サブスク名から3/10を判定し、実際のロケーション数がプランを超える場合は locationPlanMismatch でホーム・料金プラン以外へアクセスをリダイレクト、画面上部に案内バナー、料金プラン画面で「Shopifyでプランを変更する」リンクを表示。10ロケーション超で idempotencyKey 付き Usage 報告。料金プランページ「このプランを選択する」は POST→confirmationUrl リダイレクト。設計: docs/PUBLIC_APP_PLAN_FEATURES_DESIGN.md セクション8） |
| **必須コンプライアンスWebhook（App Store審査）** | ✅ 完了（2026-02。Shopify App Store 審査で必須の **customers/data_request**・**customers/redact**・**shop/redact** を購読。shopify.app.toml / shopify.app.public.toml に `compliance_topics` と `uri = "/webhooks/compliance"` を追加。app/routes/webhooks.compliance.tsx を新規作成し、authenticate.webhook で HMAC 検証・不正時 401、各トピックで 200 返却・shop/redact 時は当該ショップの Session/InventoryChangeLog/OrderPendingLocation/RefundPendingLocation 削除・customers/redact 時は orders_to_redact に紐づく一時データ削除。**パートナー「一般的なエラーの自動チェック」通過済み**。参照: https://shopify.dev/docs/apps/build/compliance/privacy-law-compliance） |
| 管理画面：棚卸・入出庫・仕入・発注・ロス・調整のソートカード | ✅ 完了（2026-02-28。各一覧のフィルター下にソート用カードを追加。棚卸：棚卸ID 昇順/降順（デフォルト降順）。入出庫：日付 昇順/降順（デフォルト降順）。仕入・発注・ロス・調整：各ID（#B0001/#P0001/#L0001/#A0001）昇順/降順（デフォルト降順）。「棚卸ID順に並び替え」ボタンは削除。実装: app.inventory-count.tsx, app.history.tsx, app.purchase.tsx, app.order.tsx, app.loss.tsx, app.adjustment.tsx） |
| 棚卸：棚卸ID番号重複防止（新規作成時） | ✅ 完了（2026-02-28。現行実装：create_inventory_count で getNextCountNumber（NEXT_KEY→バックアップ→list/main の max+1）で次の番号を取得し、appendNewCountToChunked で1件末尾に追加。追加後に NEXT_KEY を「付与した番号＋1」に更新。通常時は過去のID名称と重複しない。複数タブ・複数ユーザーが同時発行した場合のみ同一 countName の重複リスクあり。repair_count_names や countName 変更時は同一 countName で id が異なるものを検出して保存を拒否。docs/STOCKTAKE_COUNTNAME_AUDIT.md, METAFIELD_UPDATE_ISSUES.md。実装: app.inventory-count.tsx） |
| POS棚卸：Throttled エラー時のリトライ・メッセージ改善 | ✅ 完了（2026-02-28。writeInventoryCounts 内の graphql 呼び出しを runWithThrottleRetry でラップし、Throttled 時は最大2回・2秒遅延でリトライ。バッチ間は BATCH_WRITE_DELAY_MS（150ms）を挿入。保存失敗時に「Throttled」の場合は「サーバー負荷により一時制限されています。しばらく待ってから再度「確定」を押してください。」を表示（formatSaveError）。実装: stocktakeApi.js, InventoryCountList.jsx） |
| 棚卸：「商品グループをすべて完了にする」ボタン非表示 | ✅ 完了（2026-02-28。発行タブのモーダル内「商品グループをすべて完了にする」ボタンを条件付きで非表示に変更。実装: app.inventory-count.tsx） |
| POS棚卸アプリタイル一覧：作成日降順で表示 | ✅ 完了（2026-02-28。InventoryCountConditions.jsx で一覧を createdAt の降順でソート。管理画面は棚卸ID降順、アプリタイルは作成日降順で表示。実装: extensions/stock-transfer-stocktake/src/screens/stocktake/InventoryCountConditions.jsx） |
| POS棚卸：単一商品グループで商品リストが読み込まない不具合 | ✅ 完了（2026-03-05。①Conditions：onTapCount・handleShowAllProductGroups で productGroupIds が無い場合は productGroupId から配列を生成。②InventoryCountList：loadProducts で locationId = c?.locationId ?? count?.locationId でフォールバックし、fetch・下書き・在庫取得で統一使用。③ProductGroupSelection：resolvedProductGroupIds（productGroupId フォールバック）を useMemo で算出しグループ名・一覧・数量読込で共通利用。単一タップ・複数→1グループ選択・まとめて表示の3経路で商品リスト読み込みを同一化。実装: InventoryCountConditions.jsx, InventoryCountList.jsx, InventoryCountProductGroupSelection.jsx） |

---

## 実装情報の追記（2026-03）

棚卸まわりの挙動検証とドキュメント整備に基づき、以下の実装・対策を要件書に反映した。

| 項目 | 内容 | 参照 |
|------|------|------|
| **POS棚卸：単一商品グループで商品リストが読み込まない不具合** | コンディション画面で棚卸IDタップ時、API/ストレージで productGroupIds が無く productGroupId のみの count の場合に商品リストに遷移できず「商品グループが見つかりません」となる事象を修正。①**Conditions**：onTapCount と handleShowAllProductGroups で productGroupIds が無い場合は productGroupId から `[c.productGroupId]` を生成。②**InventoryCountList**：loadProducts で `locationId = c?.locationId ?? count?.locationId` とし、fullCount に locationId が無い場合も親の count でフォールバック。fetch・下書きキー・在庫一括取得で locationId を統一使用。③**ProductGroupSelection**：resolvedProductGroupIds（productGroupId フォールバック付き）を useMemo で算出し、グループ名取得・グループ一覧・数量読込で共通利用。これにより単一タップ・複数グループから1グループ選択・まとめて表示の3経路で商品リスト読み込みが同一ロジックで動作。 | InventoryCountConditions.jsx, InventoryCountList.jsx, InventoryCountProductGroupSelection.jsx |
| **確定処理（差異あり/なし・3表示モード）** | 差異あり・差異なしのどちらも、商品グループごと表示・まとめて表示・単一商品グループのみのいずれでも問題なく処理される。buildUpdatedCountFromLocalState で groupItems を組み立て、buildCompletedGroupsPayload → reportStocktakeCompleteToApi。API は既存 groupItems に送信分をマージするため、1グループだけ送っても他グループは消えない。 | docs/STOCKTAKE_CONFIRM_VERIFICATION.md「確定処理：表示モード別・差異あり/なしの対応」 |
| **確定成功まで下書きが消えない** | clearAllInventoryCountDraftsForCount は4経路すべてで「API が ok を返した後」かつ「差異ありのときは在庫調整も成功した後」にのみ呼ばれる。失敗時は else/catch でエラートーストのみで clear は実行されない。 | 同上「確定ボタン押下後：成功するまで自動保存（下書き）が消えないか」 |
| **棚卸ID発行時の過去名称との重複** | getNextCountNumber で NEXT_KEY／バックアップ／list の max+1 を参照。appendNewCountToChunked で1件追加し、追加後に NEXT_KEY を更新。通常時は過去のID名称と重複しない。同時発行時のみ同一 countName の重複リスク。名前変更・修復時は同一 countName で id が異なるものを検出して保存拒否。 | docs/STOCKTAKE_COUNTNAME_AUDIT.md, docs/METAFIELD_UPDATE_ISSUES.md |
| **ステータスダウングレード防止** | mergeExistingNonBlank で、既存が completed または cancelled のときは payload の in_progress/draft で上書きせず既存の status・completedAt を維持。過去の「完了→未処理に戻る」事象の要因を残さない。 | app.inventory-count.tsx mergeExistingNonBlank, docs/METAFIELD_UPDATE_ISSUES.md |
| **空でのメタ上書き防止** | writeInventoryCountsChunked で payloads.length === 0 のとき、メインキー確認で例外（ネットワーク等）が発生しても「棚卸データの状態を確認できませんでした。空での上書きはブロックしました。」を throw し、空での上書きを行わない。 | app.inventory-count.tsx writeInventoryCountsChunked, docs/METAFIELD_UPDATE_ISSUES.md |
| **履歴表示のフォールバック** | loader で list が空かつ main がチャンク形式のとき、main チャンクから一覧を取得する fetchMainChunksViaLoader を実行し、ID発行直後や list 未更新時も履歴に表示される。 | app.inventory-count.tsx loader, docs/METAFIELD_UPDATE_ISSUES.md |
| **POS 読み込み速度** | 単一グループ・まとめて表示の商品リスト読み込みを、商品グループごと表示と同様の経路（readProductGroups のキャッシュ・fetchProductsByGroups のオプション統一）に揃えた。まとめて表示の完了/キャンセル済みグループは Promise.all で並列取得。 | InventoryCountList.jsx, docs/STOCKTAKE_POS_LOAD_SPEED.md |
| **管理画面 未完了グループ読み込み** | 履歴モーダルで未完了グループの商品リストを、3つの useFetcher とキューで並列取得（最大3グループ同時）。モーダルオープン時に未完了 groupId をキューに積み、各 fetcher のレスポンスで次をキューから取り出して submit。 | app.inventory-count.tsx（incompleteGroupProductsFetcher2/3, incompleteGroupQueueRef 等） |
| **メタフィールド復元UIの非表示** | 管理者用メタフィールド復元の入力枠・「表示」ボタン・修復セクションを SHOW_METAFIELD_RESTORE_UI = false で非表示。コードは残しており、将来の再有効化が可能。 | app.inventory-count.tsx |

---

## 実装まとめ（2026-02）

要件書で記載している機能・修正の実装を、役割別に整理した一覧です。詳細は上記「要件の再まとめ」表および「このチャットで決めた要件・実装と進捗」を参照してください。

### 実装の全体像

| 領域 | 内容 | 主な実装場所 |
|------|------|----------------|
| **管理画面（React Router）** | 入出庫・ロス・棚卸・仕入・発注・調整の履歴一覧・モーダル・CSV、在庫高・在庫変動履歴、設定・ホーム・料金プラン | `app/routes/app.*.tsx`（history, inventory-count, adjustment, purchase, order, inventory-info, settings, plan, _index 等） |
| **POS 拡張（Preact）** | 出庫・入庫・ロス・棚卸・調整・発注・仕入の各タイル。カメラスキャン・商品リスト・コンディション・確定処理・変動記録 | `extensions/stock-transfer-tile`, `stock-transfer-inbound`, `stock-transfer-loss`, `stock-transfer-stocktake`, `stock-transfer-adjustment`, `stock-transfer-order`, `stock-transfer-purchase` |
| **API・Webhook** | 変動記録API、在庫スナップショットCron、コンプライアンスWebhook、救済API | `app/routes/api.*.tsx`, `webhooks.*.tsx` |
| **設定・課金・公開** | アプリ表示件数（3項目）・CSV項目設定・予定外棚卸許可、プラン取得・サブスク・Usage、公開/自社デプロイ | `app.settings.tsx`, `app/utils/billing.ts`, `scripts/set-app-mode.js`, `deploy:public` / `deploy:inhouse` |

### 機能別サマリー

| 機能 | 状態 | 主なファイル・備考 |
|------|------|---------------------|
| **入出庫** | ✅ 完了 | 履歴・モーダル（配送単位・進捗・予定外ブロック）、出庫確定後コンディション遷移・下書き削除統一、到着予定日ボタン設定。ModalOutbound.jsx, app.history.tsx |
| **棚卸** | ✅ 完了 | ID発行・未完了グループ商品取得（キー正規化・GID正規化・読み込み中表示）、まとめて表示安定化・CSVグループ対応、完了/処理中不整合解消・削除済みグループ完了扱い。POS: readInventoryCountsFirstPage/Page・InventoryCountConditions 分割取得・一覧用listメタフィールド・インデックス参照で readInventoryCountById 軽量化。**2026-02-27** 在庫数＝実数で差分なし時の確定UX（確認モーダルで確定するで完了・トーストは「棚卸を完了しました」のみ・モーダル文言2行化。InventoryCountList.jsx）。**2026-02-27** ロケーションに在庫レベルがない時の確定失敗を解消（ensureInventoryActivatedAtLocation で在庫有効化してから inventorySetQuantities。stocktakeApi.js）。**2026-02-28** 在庫差異なし時もUI先完了・確定後遷移でステータス即時反映（差異なし2経路も即時 toast/onAfterConfirm/setSubmitting の後バックグラウンドwrite・直列化。確定後は全完了→コンディション/未完了あり→商品グループリストへ遷移。InventoryCountList.jsx, StocktakeScreen.jsx）。**2026-02-28** アプリタイルリスト・確定処理高速化・完了表示・merge安全化（handleAfterConfirm(null)時は何もしない・refresh失敗時setCounts([])しない・全4経路でUI先完了→バックグラウンドwrite・下書きは確定成功時のみ削除・pendingBackgroundWriteByCountIdで直列化・fixCountsStatusOnly/readでcompletedは上書きしない・mergeでIDをString比較・allCountsをArrayガード。StocktakeScreen.jsx, InventoryCountConditions.jsx, InventoryCountList.jsx, InventoryCountProductGroupSelection.jsx, stocktakeApi.js）。**2026-02-28** 在庫有効化エラー時「在庫有効化に失敗しました」に統一・有効化成功率向上（追跡ON失敗時も activate 試行・リトライ4回・遅延延長・アイテム間150ms・リトライ時は数量なし activate・already has を既に有効として成功・失敗時は除外せず throw。stocktakeApi.js）。**2026-02-26** 一覧・商品グループ・管理画面履歴の「N件 N/N」をバックグラウンド取得（POS棚卸一覧は readInventoryCountById 行ごと・取得中は「…」、POS商品グループ一覧は表示時に loadProductGroupQuantities 自動実行、管理画面履歴一覧は listRowDetailFetcher で get_count_full をキューで1件ずつ・listCountDetails にマージ・行クリック時は取得済みをモーダルに渡す）。**2026-02-26** まとめて表示の fetchProductsByGroups 戻り値を配列に正規化し D.map is not a function を解消（InventoryCountList loadGroupProducts）。**2026-02-26** まとめて表示の null is not an object (evaluating '_n.map') を解消（loadProducts 内の全経路で fetchProductsByGroups 戻り値を配列正規化）。**2026-02-26** まとめて表示の null.map 追加対策（キャンセル済みで商品0件のとき completedItems が null で .map が落ちる不具合を (completedItems || []).map で防止。stocktakeApi の fetchProductsByGroups で rawGroups を配列に正規化して groups で .filter が落ちないようにする。InventoryCountList.jsx, stocktakeApi.js）。**2026-02-26** 在庫＝実数で差分なしでも確定で完了（まとめて表示で groupItems に全グループ保存・トースト「在庫数に変更はありませんが、このまま確定して完了できます」・確定後「棚卸を完了しました」のみに統一）。**2026-02-26** 商品リスト250件以上で「さらに読み込み」が出ない対策（hasMore 初回 effectiveFirst 以上で true）。**2026-02-26** 商品リスト読み込み中の「商品がありません」チラつき防止（loadCompletedRef）。**2026-02-26** 棚卸ID採番の安定化・誤採番の自動修正（createdAt+id ソート・管理画面も同順で採番・countName 常時再計算で既存の誤りも修正）。**2026-02-26** 商品グループの表示を skus（CSV登録順）に統一（resolveSkusToInventoryItemIds の戻り順・管理画面/POSの表示ソート）・管理画面で並び順説明追加・プレースホルダー「例: 雑貨グループ」。**2026-02-26** CSVアップロード後の表示リストのカード枠・間隔を商品検索と統一（スクロール枠 padding 6px・カード間隔・listIndex・borderBottom 削除）。**2026-02-26** 商品グループ編集で getVariantsByInventoryItemIds/getVariantsBySkus を nodes(ids) 250件チャンクに分割（SKU件数が多いと商品リストが読み込めない不具合を修正。NODES_IDS_MAX=250）。**2026-02-26** 商品検索・CSVから作成をマストとするため fetchProductsByGroups で inventoryItemIdsByGroup が無い場合に group.inventoryItemIds をフォールバック使用（CSV由来・コレクションなしでも商品リスト・在庫数読込が動作）。**2026-02-26** 管理画面：棚卸ID発行を「グループを追加」と同じUI（白カード内見出し・青ボタン）。**2026-02-26** POS：商品グループ一覧でステータス（処理済み/キャンセル/未処理）を count から即時表示、在庫数は読込ボタンで取得（getCancelledGroupIdSet を export）。**2026-02-25** 商品リスト「さらに読み込む」をコレクション経路でも pageInfo/after で250件超取得可能に・下書き復元時 hasMoreProducts 再設定・まとめて表示読込の c→count 修正・商品グループ一覧の在庫数読込をグループ順次・15件バッチ化。**2026-02-25** 追加読み込みのタイムアウトを初回40秒・追加60秒に延長・読み込みボタン押下直後に「読込中...」即時表示。**2026-02-25** 管理画面: loader で list 優先・チャンク並列取得・リトライ、action は create_inventory_count 時のみ棚卸フル取得、モーダル list-only 時「商品リストを読込中...」、CSVアップロードしたグループ編集で getVariantsBySkus により商品リスト表示。**2026-02-25** 管理画面キャンセル対応でPOSアプリタイルに影響なし（cancelledGroupIds を完了判定に含める・status=cancelled は上書きしない・toMinimalCountForList に cancelledGroupIds/productGroupNames・InventoryCountList の cancelledGroupIdSet/allDone/newStatus）。**2026-02-25** 管理画面モーダル：編集ボタン押下後の状態維持（リセットはモーダル閉じ時のみ）・再オープン時に古い fetcher.data を無視（lastSubmittedForModalRef）・同一ショップ複数ブラウザ時の負荷軽減（get_incomplete_group_products のサーバー側キャッシュ・TTL2分・更新時無効化）。**2026-02-25** POS棚卸：まとめて表示・グループごと表示で完了/キャンセル済みグループは groupItems を優先し未完了のみ下書き。**2026-02-28** 棚卸IDが空白で上書きされないよう write 前に ensureCountNamesBeforeWrite（stocktakeApi.js）・Conditions のバックグラウンドマージで full.countName 反映（InventoryCountConditions.jsx）。**2026-02-28** 管理画面で「棚卸IDを修復」機能（発行タブにボタン・action repair_count_names・ensureCountNamesOnCounts。docs/STOCKTAKE_COUNTNAME_AUDIT.md）。**2026-02-28** 管理画面ソートカード（棚卸・入出庫・仕入・発注・ロス・調整のフィルター下・昇順/降順・デフォルト降順）・棚卸ID重複防止（create_inventory_count で保存直前 readInventoryCountsChunked→ensureCountNamesOnCounts）・POS Throttled リトライ・バッチ間遅延・保存失敗メッセージ改善・「商品グループをすべて完了にする」非表示・アプリタイル一覧は createdAt 降順。**2026-02-28 確定まわり追補**：単一グループでステータス「完了」になるよう resolvedAllIds/groupIdsForCheck・adjustInventoryToActual に Throttled リトライ・確定後メタ read/write に runWithBackgroundWriteRetry（最大3回）・writeInventoryCounts で既存不足分を merged に補完・単一グループ下書きは count 単位キー（LEGACY フォールバック）・fromStorage ?? count と read リトライ。検証: docs/STOCKTAKE_CONFIRM_VERIFICATION.md, STOCKTAKE_DRAFT_LOSS_CAUSES.md。app.inventory-count.tsx, extensions/stock-transfer-stocktake。docs: STOCKTAKE_APP_TILE_LOAD_PERFORMANCE_ANALYSIS.md, STOCKTAKE_UX_TAP_AND_LOAD_BUTTONS.md, STOCKTAKE_COUNTNAME_AUDIT.md, ADMIN_INVENTORY_COUNT_MULTI_BROWSER_LOAD.md, ADMIN_STOCKTAKE_MODAL_LOAD_AND_CONFIRM_CANCEL.md, STOCKTAKE_5SEC_SPEC_ESTIMATE.md, STOCKTAKE_DRAFT_AND_READONLY_QA.md |
| **棚卸デバッグ** | ✅ 対応済 | get_incomplete_group_products の GraphQL errors ログ出力、action catch で graphQLErrors 出力。手順: docs/STOCKTAKE_DEBUG_PRODUCT_LIST_LOADING.md |
| **ロス・発注・仕入・調整** | ✅ 完了 | 各履歴・商品リスト・CSV項目設定。調整は簡易棚卸（activity=adjustment）。商品リストの初回表示件数は productList.initialLimit に統一。**2026-02-26** スキャン処理を棚卸・入庫と同様に統一（即リスト反映＋トースト、在庫はバックグラウンド）。既存行の数量追加・検索追加・+-変更時は在庫取得しない（linesRef で新規行のみ fetchVariantAvailable）。**2026-02-25** 履歴詳細の商品リストで設定の初回表示件数と「さらに読み込む」を追加（*HistoryList.jsx）。各 extensions の *ProductList.jsx, *HistoryList.jsx, app.settings.tsx |
| **アプリ表示件数** | ✅ 完了 | 設定で「履歴一覧リスト・商品リスト・検索リスト」の3項目。全タイルで反映。デフォルト: 100 / 250 / 50。docs/APP_DISPLAY_COUNT_SETTINGS_BY_TILE.md |
| **アプリタイル一覧のロケーション・並び順・タップ後詳細の統一** | ✅ 完了（2026-02-25） | 棚卸・仕入・発注・ロス・調整の5タイルで、ロケーションはログイン中のみ表示（GID/数値正規化・棚卸は0件フォールバック削除）、並び順は日付降順、一覧は軽く・詳細はタップ後にrefから表示（入庫・出庫と同様の操作感）。実装: 各 extensions の *HistoryList.jsx, InventoryCountConditions.jsx。docs/APP_TILE_LIST_LOAD_AND_LOCATION_FIX.md |
| **メタフィールド分割取得** | ✅ 完了 | 発注・仕入・ロス・調整・棚卸の履歴で first/after によるページネーション取得。各履歴 loader・API |
| **在庫高・変動履歴** | ✅ 完了 | スナップショット共通化・Cron・トークンリフレッシュ、変動はDB・OrderPendingLocation/RefundPendingLocation・売上/返品/キャンセル戻り/調整反映・二重防止・救済API。**2026-02-28** 在庫情報画面：useMemo is not defined 対策（React.useMemo に変更）・loader エラー時フォールバック返却・loaderData.error 時は簡易エラーメッセージ表示。app.inventory-info.tsx, api.log-inventory-change, webhooks.*, app/utils/inventory-snapshot.ts |
| **設定画面** | ✅ 完了 | 破棄・保存を下部固定フッター、左にステータス表示。表示件数説明は「最大・推奨」のみ。app.settings.tsx |
| **ホーム・料金** | ✅ 完了 | スマホでプラン・ロケーション数を最上部表示、POS設定/チャネルリンク。4プラン・ロケーション不一致時リダイレクト・案内。Pro $120(10loc)/$12(従量)。app._index.tsx, app.plan.tsx, app/utils/billing.ts |
| **審査対応** | ✅ 完了 | customers/data_request, customers/redact, shop/redact 購読・webhooks.compliance。パートナー自動チェック通過済み。app/routes/webhooks.compliance.tsx |

### 主要ファイル一覧（参照用）

- **管理画面**: `app/routes/app.settings.tsx`, `app.inventory-count.tsx`, `app.history.tsx`, `app.adjustment.tsx`, `app.purchase.tsx`, `app.order.tsx`, `app.inventory-info.tsx`, `app.plan.tsx`, `app._index.tsx`
- **POS 拡張**: `extensions/stock-transfer-tile/src/ModalOutbound.jsx`, `extensions/stock-transfer-stocktake`（stocktakeApi.js, InventoryCountList.jsx）, 各タイルの *ProductList.jsx
- **API・Webhook**: `app/routes/api.log-inventory-change.tsx`, `api.inventory-snapshot-daily.tsx`, `api.reclassify-change-history.tsx`, `webhooks.inventory_levels.update.tsx`, `webhooks.orders.updated.tsx`, `webhooks.refunds.create.tsx`, `webhooks.compliance.tsx`
- **共通・ユーティリティ**: `app/utils/inventory-snapshot.ts`, `app/utils/inventory-change-log.ts`, `app/utils/billing.ts`, `app/utils/admin-webhook-retry.ts`, `extensions/common/logInventoryChange.js`, `extensions/common/appUrl.js`
...
| **①在庫高** | 日付選択の枠がスマホで領域をはみ出している。 | 日付入力の親要素に `width: 100%`, `minWidth: 0`, `overflow: hidden` を指定し、領域内に収まるよう修正（`app.inventory-info.tsx`）。**2026-02-12 追記**：実機のアプリブラウザで日付枠が切れる・本日集計ボタンが改行される事象に対し、フィルターカード・日付+ボタン行・左カラムに `minWidth: 0` / `overflow: hidden` / `flexWrap: nowrap` を付与し、同一行ではみ出さないように再修正。詳細は「直近の主な更新」の当該項目参照。 |
...
| **②変動履歴** | 履歴の項目を、商品名・オプションを含め他商品リスト・CSV と同等にしたい。「変動量」→「変動数」に変更したい。 | **一覧・CSV**：表示項目を「発生日時・商品名・SKU・オプション・ロケーション・アクティビティ・**変動数**・**変動後在庫数**・参照ID・備考」に変更（2026-02-13 に変動後数量→変動後在庫数に文言変更）。商品名・オプションはバリアントを GraphQL で一括取得して付与。CSV はオプションを「オプション1」「オプション2」「オプション3」列で出力。 |
...
| **②変動履歴（2026-02-19）** | 入庫なのに備考「変動数は直前ログが存在しなかったため記録されていません」が残る／特定店舗の入庫がすべて「管理」になる。 | **要因・恒久対策・救済**：①**備考**：Webhook が先に delta=null で保存した行を API が上書きする際、**note を null にクリア**するよう修正（api/log-inventory-change）。②**「管理」のまま**：admin_webhook 検索の **recentFrom を「現在から最大15分前」まで遡る**よう拡張（API が数分遅れても既存行を拾って入庫等に上書き）。③**既存データ救済**：`POST /api/reclassify-change-history` で「管理」→入庫等の一括振り直し・備考クリアが可能。要因・手順は `docs/INVENTORY_HISTORY_ISSUES_20260219.md`。 |
...
| 区分 | 内容 | 状態 |
|------|------|------|
| **棚卸・管理画面（2026-02-28）** | ①**管理画面ソートカード**：棚卸・入出庫・仕入・発注・ロス・調整の各一覧でフィルター下にソートカードを追加（棚卸は棚卸ID、入出庫は日付、他は各IDの昇順/降順、デフォルトは降順）。「棚卸ID順に並び替え」ボタンは削除。②**棚卸ID番号重複防止**：create_inventory_count で保存直前に readInventoryCountsChunked で最新一覧を取得し、ensureCountNamesOnCounts で付与（同時発行での重複を防止。docs/STOCKTAKE_COUNTNAME_AUDIT.md に「番号重複の防止」追記）。③**POS棚卸 Throttled 対策**：runWithThrottleRetry（最大2回・2秒遅延）・バッチ間150ms遅延・保存失敗時メッセージ改善（formatSaveError）。④「商品グループをすべて完了にする」ボタン非表示。⑤POS棚卸アプリタイル一覧は createdAt 降順で表示。実装: app.inventory-count.tsx, app.history.tsx, app.purchase.tsx, app.order.tsx, app.loss.tsx, app.adjustment.tsx, stocktakeApi.js, InventoryCountList.jsx, InventoryCountConditions.jsx。 | ✅ 完了 |
...
| **変動履歴** | **オンラインストア売上**：受注直後は FulfillmentOrder.assignedLocation が null になる仕様のため、`orders/updated` でロケーションが取れず「管理」のままになる問題に対応。`OrderPendingLocation` テーブルを追加し、ロケーションが取れない場合は line_items を一時保存。`inventory_levels/update` で同一商品・時刻が近い保留注文と突き合わせて `order_sales` で保存。Prisma に `OrderPendingLocation` モデル、マイグレーション `20260211000000_add_order_pending_location` を追加。 | ✅ 完了 |
| **変動履歴** | **売上判定の救済**：`inventory_levels/update` が先に届いて admin_webhook で保存された場合、後から届いた `orders/updated` で直近の admin_webhook を検索し `order_sales` に上書きする救済処理を追加（ロケーションなし時）。連続取引・Webhook 到達順で判定漏れするケースを補完。 | ✅ 完了 |
| **変動履歴** | **FLOW 等の別イベント計測**：仕入の直後に FLOW で在庫を 0 に戻すなど、既存ログの `quantityAfter` と今回の `available` が異なる場合は「別イベント」とみなし、新規で「管理」行を作成するように変更（`inventory_levels/update`）。同一イベントの追報のときのみ既存行の quantityAfter を更新。 | ✅ 完了 |
| **変動履歴** | **POS 売上/返品の二重登録防止**：`api/log-inventory-change` の admin_webhook 検索で `recentTo` を「リクエスト ts+5分」と「現在+2分」の遅い方に拡大。既存の order_sales/refund が同一 item・location・時間にいる場合は新規作成せずその行の quantityAfter 等を更新するように変更。orders/updated が先に admin_webhook を order_sales に更新したあとに API が届いても二重にならない。 | ✅ 完了 |
| **変動履歴** | **返品の同様反映**：`refunds/create` の admin_webhook 検索の `searchTo` を「返品作成+5分」と「現在+2分」の遅い方に拡大。inventory_levels/update が先に届いた場合でも後から届く refunds/create で admin_webhook を refund に更新しやすくする。 | ✅ 完了 |
| **変動履歴** | **売上の変動数にオーダー数量を反映**：オーダー情報（`line_items[].quantity` / OrderPendingLocation の quantity）を変動数（delta）に使用。`inventory_levels/update` で OrderPendingLocation とマッチした場合は `delta = -pendingOrder.quantity` で保存。`orders/updated` で admin_webhook を order_sales に更新する際は `delta = -(lineItem.quantity)` で統一。オンライン・POS ともに履歴の「変動数」列に売上点数が表示される。 | ✅ 完了 |
| **変動履歴** | **売上・返品：受注時優先・到着順非依存・POS救済（2026-02-12）**：①受注時（fulfillments=0）で常に OrderPendingLocation を登録し、後から届く inventory_levels/update とマッチして order_sales＋変動数で記録。②配送完了時（fulfillments あり）は、既にこの注文の order_sales が1件でもあれば救済をスキップ（オンラインは受注時で反映済みのため）。③fulfillments ありで admin_webhook がまだ無い場合（POS で orders/updated が先に届く等）は OrderPendingLocation に登録し、後から inventory_levels/update でマッチ。④admin_webhook を order_sales に更新した際は該当 OrderPendingLocation を削除して二重記録防止。⑤販売可能数と手持ち在庫数は分けない方針で運用。詳細・届く情報の差・検証チェックリストは `docs/SALES_REFUND_ACTIVITY_AND_WEBHOOK_API_FLOW.md`・`docs/SALES_REFUND_VERIFICATION_CHECKLIST.md` 参照。 | ✅ 完了 |
| **変動履歴** | **アクティビティ「管理」残り対策（2026-02-12）**：履歴とRenderログから、18:29（複数明細の5本目だけ救済漏れ）・19:30（同一商品の連続売上2件目が「別イベント」で管理になる）の要因を分析。`inventory_levels/update` で admin_webhook を保存する**直前に** OrderPendingLocation を再検索し、その時点で登録されていれば order_sales で保存するように変更。レースや連続売上で「管理」が残るケースを軽減。要因分析は `docs/INVENTORY_ACTIVITY_MANAGEMENT_CAUSE_20260212.md`。実装: `webhooks.inventory_levels.update.tsx`。 | ✅ 完了 |
| **在庫高** | **日付枠・本日集計ボタンのスマホレイアウト（2026-02-12）**：実機のアプリブラウザで日付入力が切れる・本日集計が改行される事象に対応。フィルター白カード・「日付＋本日集計」行・左カラムに `minWidth: 0` / `overflow: hidden` / `flexWrap: nowrap` / `maxWidth: 100%` を付与し、日付とボタンを同一行で表示しはみ出さないように修正。実装: `app.inventory-info.tsx`。 | ✅ 完了 |
| **変動履歴** | **既存 admin_webhook を order_sales に更新して二重「管理」防止（2026-02-12）**：OrderPendingLocation にマッチした場合、新規行を作る前に「同一 item/location で quantityAfter = available + 売上数量 の admin_webhook が時間窓内にないか」を検索。見つかったらその既存行を order_sales に更新し新規行は作らず return（20:11/20:14 型の同一商品で「管理」が2行並ぶ事象を防ぐ）。実装: `webhooks.inventory_levels.update.tsx`。 | ✅ 完了 |
| **変動履歴** | **注文編集時の救済で時間窓を updated_at 基準に変更（2026-02-12）**：fulfillments なし（注文編集等）時の admin_webhook→order_sales 救済の時間窓が注文作成日時（created_at）基準だったため、昔に作成された注文を編集した場合に編集直後の admin_webhook が窓に入らず救済されない問題を修正。救済の searchFrom/searchTo を**注文更新日時（updated_at）**基準に変更。実装: `webhooks.orders.updated.tsx`。 | ✅ 完了 |
| **変動履歴/在庫高** | **変動一覧リスト下の注釈文言（2026-02-12）**：「管理画面からの在庫数量変更：対象SKUとロケーションの変動が初回の場合は変動数が「-」表記になります。」に変更（改行＋「変動数が」を追記）。実装: `app.inventory-info.tsx`。 | ✅ 完了 |
| **在庫高** | **本日集計で本日選択時に全ロケーション表示するよう修正（2026-02-12）**：本日を選択したときにロケーションが1つしか表示されない不具合を修正。原因は `todaySnapshot = savedSnapshots.snapshots.find(...)` で先頭1件だけを使い `currentInventory = [todaySnapshot]` としていたため。本日分も `savedSnapshots.snapshots.filter((s) => s.date === todayInShopTimezone)` で全件取得するように変更。要因・検証は `docs/INVENTORY_INFO_TODAY_SNAPSHOT_ONE_LOCATION.md`。実装: `app.inventory-info.tsx`。 | ✅ 完了 |
| **在庫高 Cron（2026-02-13）** | ①**認証**：Cron が 401 Unauthorized になる要因として、**Web サービス（stock-transfer-pos）の Environment に `INVENTORY_SNAPSHOT_API_KEY` が必須**。Cron Job 側と同一値を設定すること。②**最終確定**：既にその日付のスナップショットがあってもスキップせず**上書き**するよう変更（Cron を最終確定として扱う）。③**日付**：ショップ時間で 23 時台に実行されたときは「今日」の日付で保存、それ以外は「昨日」の日付。④**並列処理**：複数ショップを同時処理（デフォルト 3 店ずつ）。環境変数 `SNAPSHOT_CONCURRENCY`（1〜10）で変更可能。実装: `api.inventory-snapshot-daily.tsx`。ドキュメント: `docs/CRON_JOB_SETUP_GUIDE.md` に SNAPSHOT_CONCURRENCY の説明を追加。 | ✅ 完了 |
| **在庫変動履歴（2026-02-13）** | ①**ページネーション**：表示を 5000 件/ページにし、入出庫履歴と同様の UI（表示範囲・総件数・前へ/次へ）。URL パラメータ `changeHistoryPage`。フィルター適用時は 1 ページ目へリセット。②**CSV 出力**：検索結果**全件**をダウンロード（最大 50,000 件。超過時はエラーで「期間や条件を絞ってください」と案内）。アクション `exportChangeHistoryCsv`、ボタン表記は「CSV出力」のまま。③**注釈**：変動一覧下に「管理画面からの在庫数量変更：対象SKUとロケーションの変動が初回の場合は変動数が「-」表記になります。」＋「CSVダウンロード制限：一括処理最大50,000件となっています。超過する場合は検索条件にて件数のご調整をお願いします。」を表示。④**ACTIVITY_LABELS** を定数化（一覧・CSV・フィルターで共通利用）。⑤**変動後在庫数**：テーブル・CSV の「変動後数量」→「変動後在庫数」に文言変更。⑥**getActivityDisplayLabel**：大文字小文字・空白に強く order_sales→売上 等を確実に表示。実装: `app.inventory-info.tsx`。 | ✅ 完了 |
| **変動履歴** | **変動数「-」対策・アクティビティ反映の強固化（2026-02-13）**：①**delta 算出**：直前ログの quantityAfter が null の場合、同一 item/location で quantityAfter が null でない直近1件を遡って取得し delta を計算（Webhook で変動数「-」を減らす）。②**既知アクティビティ行の delta 補完**：既知アクティビティの既存ログの quantityAfter が null のとき同一イベントとして quantityAfter を更新する処理で、その行の delta が null なら「前の確定数量」から delta を算出して一緒に更新（POS 由来で API が後から来ない場合も変動数を補完）。③**直近が admin_webhook のみのとき**：同一 item/location で 2 分前〜1 分後に admin_webhook が 1 件あり quantityAfter が今回の available と一致または null なら、その行を更新して新規「管理」を作らない（同一変動の 2 本目で二重防止）。④**knownActivities に purchase_cancel 追加**。⑤**注文キャンセル時の在庫戻り**：orders/updated で cancelled_at のときスキップせず、在庫戻りを activity=order_cancel で記録。既存の admin_webhook を時間窓内で検索し、あればその行を order_cancel に更新して二重防止。⑥**api/log-inventory-change**：入庫・出庫・ロス・棚卸・仕入・仕入キャンセルで、同一 item/location/activity の既存行が時間窓内にあれば新規 create せずその行を更新（POS 重複送信で二重にならない）。⑦**order_cancel** のラベル・フィルター・型を一覧に追加（履歴UIでは order_cancel→「返品」、purchase_cancel→「仕入」に表示統一）。型は `InventoryChangeLogData` に order_cancel を追加。実装: `webhooks.inventory_levels.update.tsx`、`webhooks.orders.updated.tsx`、`api.log-inventory-change.tsx`、`app.inventory-info.tsx`、`inventory-change-log.ts`。ドキュメント: `docs/INVENTORY_ACTIVITY_REFLECTION_GUARANTEE.md`（公開アプリ向け保証設計）、`docs/INVENTORY_POS_DELTA_AND_ACTIVITY_WEBHOOK_ANALYSIS.md`（変動数「-」とアクティビティ Webhook の要因分析）、`docs/INVENTORY_ACTIVITY_MANAGEMENT_CAUSE_20260213.md`（POS 二重・管理になる要因）。 | ✅ 完了 |
| **変動履歴** | **反映漏れ洗い出し・強固化（2026-02-13）**：①**管理画面 Webhook に変動数が含まれないこと**を公式（REST InventoryLevel リソース仕様）で整理し、`docs/INVENTORY_WEBHOOK_DELTA_OFFICIAL_AND_REFLECTION_HARDENING.md` に記載。②**アクティビティ反映漏れ・二重になりうる箇所**を全経路で洗い出し、一覧化。③**orders/updated**：ロケーション不明（orderLocationId が null）時は、他ロケーションの admin_webhook を誤って order_sales に更新しないよう**救済をスキップ**。OrderPendingLocation のみ登録し、inventory_levels/update のマッチで order_sales に記録。実装: `webhooks.orders.updated.tsx`。ドキュメント: `docs/INVENTORY_WEBHOOK_DELTA_OFFICIAL_AND_REFLECTION_HARDENING.md`、`docs/INVENTORY_ACTIVITY_REFLECTION_GUARANTEE.md` に本ドキュメント参照を追加。 | ✅ 完了 |
| **変動履歴** | **大量在庫処理での漏れ防止（2026-02-13）**：200SKU 以上の一括確定で API がタイムアウトすると未処理分が「管理」で残る問題に対応。`extensions/common/logInventoryChange.js` で **50 件チャンク送信**（`LOG_INVENTORY_CHANGE_CHUNK_SIZE = 50`）に変更。deltas を 50 件ずつに分割して複数回 `api/log-inventory-change` を呼び、各リクエストがタイムアウト以内に完了するようにし、全件が正しいアクティビティで記録されるようにした。ドキュメント: `docs/INVENTORY_WEBHOOK_DELTA_OFFICIAL_AND_REFLECTION_HARDENING.md` に「5. 大量在庫処理（200SKU・1000個等）での漏れリスクと対策」を追加。 | ✅ 完了 |
| **変動履歴** | **チャンク送信のリトライ（2026-02-13）**：一時的なネットワーク障害でチャンク送信が失敗した際、未処理分が「管理」のまま残る問題に対応。`extensions/common/logInventoryChange.js` に **sendChunkWithRetry** を追加し、失敗時に最大 2 回リトライ（1秒待機）するよう実装。初回管理画面案内を README.md に追記、監視・確認チェックを `docs/INVENTORY_ACTIVITY_REFLECTION_GUARANTEE.md` に追記。ドキュメント: `docs/INVENTORY_REFLECTION_STATUS_AND_REMEDIATION.md` を更新。 | ✅ 完了 |
| **変動履歴** | **履歴一覧のアクティビティラベル・文言改善（2026-02-13）**：①**ラベル振り分け**：仕入キャンセル（purchase_cancel）→「仕入」、キャンセル戻り（order_cancel）→「返品」に表示を統一。②**変動後数量→変動後在庫数**：一覧テーブルヘッダーと CSV ヘッダーの文言を変更。③**getActivityDisplayLabel**：DB の activity 値を表示用日本語ラベルに変換するヘルパーを追加。大文字小文字・前後の空白に強く、order_sales→売上・admin_webhook→管理などが確実に表示されるよう堅牢化。未マッピング時は「その他」表示。実装: `app.inventory-info.tsx`。 | ✅ 完了 |
| **公開/自社デプロイ** | **公開アプリとカスタムアプリの2種デプロイ手順（2026-02-13）**：①**手順ドキュメント** `docs/DEPLOY_PUBLIC_AND_INHOUSE.md` を追加。パートナーダッシュボードからデプロイする場合の注意（デプロイするアプリに合わせて APP_MODE を public / inhouse にした状態で push する）と、inhouse と public の切り替え方法を記載。②**APP_MODE 書き換えスクリプト** `scripts/set-app-mode.js` を追加（`node scripts/set-app-mode.js public` または `inhouse` で `extensions/common/appUrl.js` の APP_MODE を変更）。③**npm スクリプト** `deploy:public` と `deploy:inhouse` を追加。実行すると APP_MODE を書き換え→該当 config を有効化→`shopify app deploy` を実行し、デプロイのたびに手で appUrl.js を編集せずに切り替え可能。実装: `package.json` に `deploy:public` / `deploy:inhouse` を追加。 | ✅ 完了 |
| **公開アプリ課金** | **プラン取得・サブスク作成・10loc超Usage報告（2026-02）**：①**プラン取得**：`getShopPlan`（app/routes/app.tsx）で `currentAppInstallation.activeSubscriptions` を同一 GraphQL で取得。サブスク名に "Pro"/"Lite" を含むかで `getPlanFromActiveSubscriptions` により plan を判定。開発ストアは課金せず plan=pro。②**Billing ユーティリティ**：`app/utils/billing.ts` に `getPlanFromActiveSubscriptions`・`getUsageLineItemId`・`calculateUsageAmount`・`reportUsageRecord`（appUsageRecordCreate）・`createAppSubscription`（appSubscriptionCreate：Recurring＋Usage キャップ、Lite $40/7日 trial、Pro $120/14日 trial、10 loc 超 $12/ loc）を追加。③**10ロケーション超の従量課金**：getShopPlan 内で公開・本番・Lite/Pro・locationsCount>10 のとき、アクティブサブスクの Usage line item と currentPeriodEnd を取得し、`idempotencyKey = usage-{subscriptionId}-{currentPeriodEnd}` で reportUsageRecord を fire-and-forget 呼び出し。同一請求期間の重複請求を防止。④**料金プランページ**：app.plan.tsx の「このプランを選択する」を form POST に変更。action で createAppSubscription を実行し、confirmationUrl へ redirect。設計・実装メモ: docs/PUBLIC_APP_PLAN_FEATURES_DESIGN.md セクション8。 | ✅ 完了 |
| **出庫** | **「確定する」で常に処理中（IN_TRANSIT）（2026-02-14）**：出庫モーダルで「確定する」を押したときに、追跡情報の有無にかかわらず常に Shipment を IN_TRANSIT で作成するよう変更。以前は追跡情報ありの場合のみ「処理中」になり、追跡なしだと「配送準備完了」のままだった。実装: `submitTransferCore` 内で `hasAnyTracking` 条件を外し、常に `createInventoryShipmentInTransit` を実行（`extensions/stock-transfer-tile/src/ModalOutbound.jsx`）。 | ✅ 完了 |
| **変動履歴** | **inventory_levels/update の既知アクティビティ行 quantityAfter 更新の時間窓を60秒に短縮（2026-02-14）**：Webhook で既存の既知アクティビティ行（出庫・入庫等）が見つかった場合、新規「管理」行を作らずその行の quantityAfter を更新する二重防止の時間窓を 2 分から **60 秒**（1 分）に短縮。実装: `webhooks.inventory_levels.update.tsx`（`recentThreshold`）。 | ✅ 完了 |
| **在庫高** | **表示文言の変更（2026-02-14）**：①Cron による過去日付スナップショットの説明文を削除。②スナップショット未存在時のメッセージを「〇〇のスナップショットが見つかりませんでした。」から「**〇〇の在庫高データが見つかりませんでした。**」に変更（日付は YYYY-MM-DD → **YYYY/MM/DD**）。実装: `app.inventory-info.tsx`。 | ✅ 完了 |
| **変動履歴** | **履歴UIで「仕入-」を「仕入」に表示統一（2026-02-14）**：アクティビティ purchase_cancel（仕入キャンセル）の表示ラベルを「仕入-」から「**仕入**」に変更し、仕入・仕入キャンセルとも「仕入」で統一。実装: `app.inventory-info.tsx`（activityLabels・フィルター）。 | ✅ 完了 |
| **POSロス** | **ロス理由ボタンを右寄せに統一（2026-02-14）**：ロス登録の理由選択ボタンを、希望納品日・到着予定日のクイック日付ボタンと同様に右寄せ・折り返しレイアウトに変更。実装: `extensions/stock-transfer-loss/src/screens/loss/LossConditions.jsx`。 | ✅ 完了 |
| **在庫変動履歴（2026-02-14）** | ①**CSV出力でドメイン入力画面が開く不具合**：ボタン押下で別タブに送るフォームの `form.action` が pathname のみでクエリ（ショップ情報等）が引き継がれず認証できなかったため、`form.action = location.pathname + (location.search || "")` に変更。同一URLで送ることでスムーズにCSVダウンロード可能に。実装: `app.inventory-info.tsx`。②**オンライン売上が「管理」になるWebhook対策**：orders/updated で OrderPendingLocation を「注文ロケーション取得」より**先に**登録するよう処理順を変更し、inventory_levels/update が先に届いてもマッチしやすくした。実装: `webhooks.orders.updated.tsx`。③**売上完全反映**：inventory_levels/update で「管理」で保存する直前に、OrderPendingLocation が見つからない場合は 2.5 秒待機＋最大 2 回再検索（計最大約 5 秒）を実施。Shopify が inventory_levels/update を先に送る場合でも、orders/updated の登録を待って売上で記録する。実装: `webhooks.inventory_levels.update.tsx`（PENDING_ORDER_WAIT_MS / PENDING_ORDER_MAX_RETRIES）。ドキュメント: `docs/INVENTORY_REFLECTION_STATUS_AND_REMEDIATION.md` に待機再検索の記載を追加。 | ✅ 完了 |
| **変動履歴** | **返品を売上と同様に処理（RefundPendingLocation）（2026-02-14）**：①**RefundPendingLocation** モデルを追加（売上の OrderPendingLocation と同様）。Prisma に `RefundPendingLocation`、マイグレーション `20260214000000_add_refund_pending_location`。②**refunds/create**：line_item_id 検索失敗時に GraphQL Refund API（`refund(id)` の `refundLineItems`）で inventory_item_id を取得するフォールバックを追加。複数ID形式での line_item 検索も強化。③**RefundPendingLocation 登録**：refunds/create で返品ログ作成の前に RefundPendingLocation を先に登録（inventory_levels/update が先に届いた場合のマッチ用）。④**inventory_levels/update**：delta>0（在庫増＝返品）時に RefundPendingLocation を検索し、マッチすれば「返品」として記録。保存直前再検索・2.5秒待機＋最大2回再検索・既存 admin_webhook→refund 更新で二重防止。⑤成功時に RefundPendingLocation を削除。⑥キャンセル（order_cancel）処理は既存実装で問題なしと確認。実装: `webhooks.refunds.create.tsx`、`webhooks.inventory_levels.update.tsx`、`prisma/schema.prisma`。ドキュメント: `docs/INVENTORY_REFLECTION_STATUS_AND_REMEDIATION.md`。 | ✅ 完了 |
| **変動履歴** | **admin_webhook 未検出時リトライ 30 秒を売上・返品含む全経路に適用（2026-02-15）**：共通モジュール `admin-webhook-retry.ts` を作成し、2.5 秒×12 回＝最大 30 秒の待機＋再検索を以下に適用。①api/log-inventory-change（入庫・出庫・ロス・棚卸・仕入）、②webhooks.orders.updated（売上・order_cancel、3箇所）、③webhooks.refunds.create（返品）。見つかり次第抜けて短い応答で返る。実装: `app/utils/admin-webhook-retry.ts`、`api.log-inventory-change.tsx`、`webhooks.orders.updated.tsx`、`webhooks.refunds.create.tsx`。ドキュメント: `docs/INVENTORY_REFLECTION_STATUS_AND_REMEDIATION.md`、`docs/WEBHOOK_LINKING_ISSUES_CAUSE.md`。 | ✅ 完了 |
| **変動履歴** | **在庫変動履歴のアクティビティフィルターで「仕入」を1つに統一（2026-02-15）**：仕入（purchase_entry）と仕入キャンセル（purchase_cancel）を同じ「仕入」として扱うため、フィルターの選択肢を「仕入」1つにまとめた。選択時は purchase_entry と purchase_cancel の両方を対象に検索。実装: `app.inventory-info.tsx`（PURCHASE_ACTIVITY_KEY、expandActivityTypesForQuery、normalizeActivityTypesFromUrl、フィルター肢を1つに変更）。 | ✅ 完了 |
| **出庫** | **商品リストの配送情報モーダルで arrivalQuickDays 未定義エラーを修正（2026-02-15）**：出庫の商品リスト画面で「配送情報」ボタンを押してモーダルを開くと「Can't find variable: arrivalQuickDays」が発生する不具合を修正。OutboundList 内で `arrivalQuickDays` が定義されておらず、配送情報モーダル（OutboundConditions と同様の到着予定日ボタンUI）で参照していたためエラーになっていた。OutboundList に arrivalQuickDays の useMemo を追加し、settings から算出するように変更。実装: `extensions/stock-transfer-tile/src/ModalOutbound.jsx`。 | ✅ 完了 |
| **変動履歴** | **履歴を意図通りに扱うための修正（2026-02-16）**：①**救済時に idempotencyKey を変更しない**：orders/updated・refunds/create・inventory_levels/update で admin_webhook→order_sales/refund/order_cancel に更新する際、idempotencyKey を update に含めない。他経路で同じキーが既に使われていると P2002 で救済失敗するため。②**create 時の P2002 を成功扱い**：inventory_levels/update の create で idempotencyKey 重複（P2002）をキャッチし、ログ出力の上で 200 を返す（並列 Webhook で二重送信防止）。③**quantityAfter === available の既存行も更新**：OrderPendingLocation/RefundPendingLocation マッチ時、expectedPrevQty で見つからなければ quantityAfter === available の admin_webhook を検索し、あればその行を order_sales/refund に更新（同一商品で「管理」と「売上」が2行になる残りケースを防止）。④**待機・再検索を 3 回に増加**：PENDING_ORDER_MAX_RETRIES を 2→3（最大約 7.5 秒）。ドキュメント: `docs/INVENTORY_REFLECTION_STATUS_AND_REMEDIATION.md` に待機・リトライ一覧（30秒と OrderPendingLocation の違い）・完全化のために対応済みの修正一覧を追加。 | ✅ 完了 |
| **変動履歴** | **OrderPendingLocation に locationId 追加・fulfillments ループで履行ごと登録（2026-02-16）**：①**OrderPendingLocation に locationId 追加**：同一注文で同一商品が複数ロケーションから出荷されるケースに対応するため、スキーマに `locationId` を追加。ユニークを `[shop, orderId, inventoryItemId, locationId]`、インデックスを `[shop, inventoryItemId, locationId, orderCreatedAt]` に変更。マイグレーション `20260216000000_add_location_id_to_order_pending_location`。②**orders/updated の fulfillments あり時**：`fulfillments[0]` のみでなく**全履行をループ**し、各履行の `location_id` と `line_items`（`variant_id`, `quantity`）を収集。variant_id から inventoryItemId を GraphQL で取得し、(rawItemId, locationIdRaw) で quantity を集約。③**(item, location) 単位で救済 or OrderPendingLocation**：既存 order_sales があればスキップ。時間窓内の admin_webhook があれば order_sales に更新（quantityAfter 取得・該当 OrderPendingLocation 削除）。なければ OrderPendingLocation を upsert（locationId 付き）。④処理前にこの注文の OrderPendingLocation を deleteMany し、今回の grouped のみ登録（履行削除時の古い行を残さない）。inventory_levels/update の findMany/deleteMany にも locationId を追加。実装: `webhooks.orders.updated.tsx`、`webhooks.inventory_levels.update.tsx`、`prisma/schema.prisma`。 | ✅ 完了 |
| **棚卸** | **まとめて表示の不具合修正・発行時全グループ取得・502対策（2026-02-16）**：①**0/0表示**：完了済み時の「0/0」を削除。件数は groupItemsFromMap を優先し、0件のときは「実数 X/Y」のみ表示。②**2つ目以降の商品リスト**：発行時に全選択グループの inventoryItemIds を取得（既存がなければコレクション/SKUから取得）して inventoryItemIdsByGroup に保存。POS 側で ID 正規化・inventoryItemIdsByGroup キー照合を追加し、まとめて表示で全グループの商品リストが読めるようにする。③**502対策**：発行時の商品ID取得に22秒の時間予算を設け、グループごとに try/catch して1グループ失敗でも他は続行。要因・対策は `docs/STOCKTAKE_MULTIPLE_GROUPS_PRODUCT_LIST_CAUSE.md`。実装: `app.inventory-count.tsx`（getInventoryItemIdsForGroup・時間予算・try/catch）、`extensions/stock-transfer-stocktake`（InventoryCountList.jsx, stocktakeApi.js）。 | ✅ 完了 |
| **棚卸** | **まとめて表示の安定化（2026-02-17）**：①まとめて表示のループ前に readProductGroups() を1回だけ実行し、cachedProductGroups として全 fetchProductsByGroups に渡す（全グループが同じスナップショットを参照して初回から安定表示）。②fetchProductsByGroups に cachedProductGroups オプションを追加（渡された場合は readProductGroups をスキップ）。③未完了グループで商品0件の場合、約1秒待機して1回だけリトライ。④要因ドキュメントに「グループごとで開いたあとだと表示される理由」「まとめて表示で不備なく表示するためのコード改善」を追記。実装: `extensions/stock-transfer-stocktake`（InventoryCountList.jsx, stocktakeApi.js）、`docs/STOCKTAKE_MULTIPLE_GROUPS_PRODUCT_LIST_CAUSE.md`。 | ✅ 完了 |
| **棚卸** | **CSVグループで履歴の数量母数・商品リストが表示されない事象対応（2026-02-17）**：CSVから作成したグループ（skus のみで inventoryItemIds が未保存）で、管理画面・アプリタイルの履歴一覧に数量母数が出ない・モーダルで商品リストが出ない問題に対応。get_incomplete_group_products にパターン1bを追加：collectionIds も inventoryItemIds も無いが skus がある場合、resolveSkusToInventoryItemIds で ID 解決してから商品・在庫を取得して返す。商品検索・コレクションから作成したグループは従来どおり（同様の要因にはならない）。要因・対策は `docs/STOCKTAKE_CSV_GROUP_HISTORY_QUANTITY_CAUSE.md`。実装: `app.inventory-count.tsx`。 | ✅ 完了 |
| **変動履歴** | **OrderPendingLocation/RefundPendingLocation の locationId="" 検索対応・残存リスク整理（2026-02-17）**：①**要因**：オンライン受注直後は orders/updated でロケーションが取れず OrderPendingLocation に `locationId=""` で登録されるが、inventory_levels/update の検索で `[..., ""].filter(Boolean)` により空文字が除かれマッチしなかった。②**OrderPendingLocation**：orderLocCands から `.filter(Boolean)` を削除し、初回検索・保存直前再検索・待機後再検索の3箇所で `locationId=""` を検索に含める。③**RefundPendingLocation**：locCands に `""` を追加（防御的）。マッチ時に `locationId` を保持し、deleteMany でマッチした行の locationId を使用するよう統一。④**ドキュメント**：要因を `docs/INVENTORY_ACTIVITY_MANAGEMENT_CAUSE_20260217.md`、残存リスク一覧を `docs/INVENTORY_REFLECTION_REMAINING_RISKS.md` に追加。`docs/INVENTORY_REFLECTION_STATUS_AND_REMEDIATION.md` に #25・#26 を追記。実装: `webhooks.inventory_levels.update.tsx`。 | ✅ 完了 |
| **棚卸** | **履歴IDモーダル：商品グループごとの進捗カウント修正（2026-02-18）**：管理画面の棚卸履歴一覧でIDのモーダルを開いた際、「商品グループごとの進捗: X/Yグループ完了」のXが常にY（全グループ数）になり正しくカウントされていなかった。**要因**：「X」に `itemsByGroup.size` を使用していたが、itemsByGroup には完了・未完了の両方が入るため size は常に全グループ数になっていた。**対応**：完了数は `completedGroupsMap` で追跡しているため、表示を `Array.from(completedGroupsMap.values()).filter(Boolean).length` に変更。実装: `app.inventory-count.tsx`。 | ✅ 完了 |
| **変動履歴** | **在庫変動履歴：備考クリア・API検索窓15分拡張・救済API・要因ドキュメント（2026-02-19）**：①**備考が残る**：api/log-inventory-change で admin_webhook を入庫・出庫等に上書きする際、delta/quantityAfter を更新するときは **note を null にクリア**（「変動数は直前ログが…」が入庫行に残らない）。②**「管理」のまま残る**：admin_webhook 検索の recentFrom を「現在の60秒前」から **「イベント30分前」と「現在から最大15分前」の遅い方**に変更（API が2〜3分遅れても既存行を拾って上書き）。③**救済API**：`POST /api/reclassify-change-history` を追加。shop/locationId/時刻範囲で admin_webhook を一括で inbound_transfer 等に振り直し、または clearStaleNoteOnly で備考のみクリア。認証は RECLASSIFY_CHANGE_HISTORY_API_KEY（未設定時は INVENTORY_SNAPSHOT_API_KEY）。④**要因・手順ドキュメント**：`docs/INVENTORY_HISTORY_ISSUES_20260219.md` に不具合要因・恒久対策・救済APIのcurl例・CSVと実態を一致させる手順を記載。実装: `api.log-inventory-change.tsx`、`api.reclassify-change-history.tsx`（新規）。 | ✅ 完了 |
| **簡易棚卸（調整）・変動履歴・UI（2026-02-20）** | **簡易棚卸（調整）機能**：①**POS拡張** `stock-transfer-adjustment` を追加（ロス拡張をベースに複製）。コンディションはロケーション・日付・スタッフ・メモのみ。商品リストは在庫数＋実数で、確定時に絶対数で差分調整・`activity: "adjustment"` で変動履歴に記録。履歴は #A0001 形式、キャンセル時は在庫を元に戻す。②**管理画面**：設定に「調整設定」タブ・「調整履歴CSV出力項目設定」を追加。メニューに「調整」を棚卸の直下に配置。`app.adjustment.tsx` で調整履歴一覧・フィルター・商品リストモーダル・CSV（商品グループ列なし）。③**変動履歴**：アクティビティに「調整」を追加（`inventory-change-log.ts`・`api.log-inventory-change`・`app.inventory-info.tsx` のラベル・フィルター）。④**調整タイル**：説明から「（簡易棚卸）」を削除。⑤**デプロイ**：調整拡張の UID を一意に変更（Duplicate UID 解消）。発注 OrderConditions の `!!x ?? true` を `!!(x ?? true)` に修正。⑥**商品リストフッター**：棚卸に「明細 N」を追加。全商品リストでフッター中央に明細を含む表示に統一、2行以上は中央揃え。調整はスキャン・検索追加時は実数=1で追加し数量ボタンでカウント可能に。実装: `extensions/stock-transfer-adjustment`、`app.adjustment.tsx`、`app.settings.tsx`、`app.tsx`、`app.inventory-info.tsx`、`api.log-inventory-change.tsx`、`inventory-change-log.ts`、各拡張の FixedFooterNavBar・商品リスト。ドキュメント: `docs/SIMPLE_STOCKTAKE_ADJUSTMENT_FEASIBILITY.md`。 | ✅ 完了 |
| **管理画面・調整の改善（2026-02-20）** | ①**調整設定CSV項目に「差分」を追加**：設定の調整履歴CSV出力項目に delta（差分＝実数−在庫数）を追加。一括CSV・モーダルCSVの両方で出力。②**調整履歴一覧・モーダルを「メモ」表示に統一**：一覧リストと商品リストモーダルの情報欄を「理由」→「メモ」に変更（entry.memo / modalEntry.memo を表示）。③**商品リストモーダルをCSV設定に連動**：モーダル内テーブルの列を「調整履歴CSV出力項目設定」の並び・ON/OFFに合わせて表示（明細列のみ。差分列は実数−在庫数で計算表示）。実装: `app.adjustment.tsx`、`app.settings.tsx`。 | ✅ 完了 |
| **在庫変動履歴・返品フィルター統一（2026-02-20）** | アクティビティフィルターで「返品」が2つ（refund・order_cancel）あったのを、仕入と同様に1つの「返品」に統一。選択時は refund と order_cancel の両方を検索対象に展開（expandActivityTypesForQuery）。URL の activityTypes 正規化で refund/order_cancel をどちらも「返品」1つにまとめて解釈（normalizeActivityTypesFromUrl）。実装: `app.inventory-info.tsx`（REFUND_ACTIVITY_KEY、REFUND_ACTIVITY_TYPES、フィルター選択肢を1つに変更）。 | ✅ 完了 |
| **POS出庫・発注の確定後・UI（2026-02-21）** | ①**出庫確定後**：確定後にコンディションに戻らない・下書きが残る事象を解消。他タイル（ロス・発注等）と同様に「下書き削除→モーダル閉じる→画面遷移」の順で確実に実行するよう統一。`logInventoryChangeToApi` を try/catch で囲み API 失敗時も後処理を実行。遷移は確定ボタン側でモーダル閉じたあとに `onConfirmSuccess` を実行。②**発注・希望納品日**：入力欄と日付ボタンの間隔（gap=small）・右寄せ（justifyContent=end・wrap）を出庫の到着予定日と統一。実装: `ModalOutbound.jsx`、`OrderConditions.jsx`。 | ✅ 完了 |
| **ホーム・設定・料金まわり（2026-02-22）** | ①**ホーム・スマホ**：767px以下で現在の料金プラン・ロケーション数を最上部に100%幅で表示（order/flex）。②**ホーム・POSステップ**：「POSの設定を開く」「販売チャネル一覧を開く」で管理画面のPOS設定・チャネル一覧を新しいタブで開く（storeHandle を loader/context で渡す）。③**設定**：破棄・保存を下部固定フッターにし、左にステータス（保存しました/未保存の変更があります等）、右に破棄・保存ボタン。④**パートナー価格**：docs/PARTNER_PRICING_SETUP_STEPS.md 追加。4プラン（lite-3, lite-10, pro-3, pro-10）の手順・ハイフン・表示名にLite/Proと3/10を含める前提。⑤**ロケーション数とプラン不一致**：getMaxLocationsFromSubscriptionName でサブスク名から3/10を判定。超過時は locationPlanMismatch でホーム・料金プラン以外を /app/plan にリダイレクト、案内バナー・料金プラン画面で「Shopifyでプランを変更する」リンク。⑥**Pro料金**：10ロケーション $120/月、11ロケーション目以降 $12/ロケーションに変更（billing.ts・app.plan・関連ドキュメント）。実装: app._index.tsx, app.tsx, app.settings.tsx, app.plan.tsx, app/utils/billing.ts, docs/PARTNER_PRICING_SETUP_STEPS.md。 | ✅ 完了 |
| **棚卸（2026-02-24）** | **商品リスト読み込み安定化・完了/処理中不整合解消・数量0/-・ステータス統一・グループ削除後完了扱い**：①**管理画面**：incompleteGroupProducts のキーを文字列で統一（getIncompleteProductsForGroup）・未完了取得中は「読み込み中...」・API エラー時も lastSubmittedGroupIdRef で loading 解除・get_incomplete_group_products で locationId を GID 正規化（一覧・モーダル両方）・一覧用 incompleteGroupProductsForList の保存/参照をキー正規化（母数/- 照合漏れ防止）・完了判定で「削除済みグループは完了とみなす」。②**POS**：在庫調整なし/ありどちらでも「全て0・差分なし」のグループを groupItems に保存・readInventoryCounts の後方互換（単一&items&groupItems 空で完了）を廃止し allDone のみに統一・readProductGroups で削除済みグループを完了扱い、必要時は status をメタに書き戻し。③要因・対策は docs/STOCKTAKE_ADMIN_VS_APP_STATUS_CAUSE.md。実装: app.inventory-count.tsx、extensions/stock-transfer-stocktake（InventoryCountList.jsx、stocktakeApi.js）。 | ✅ 完了 |
| **アプリ表示件数・メタフィールド分割取得・設定説明・棚卸POS（2026-02-24）** | ①**アプリ表示件数の統一**：履歴一覧・商品リスト・検索リストの表示件数を管理画面の設定値で統一。②**メタフィールド分割取得**：発注・仕入・ロス・調整・棚卸の各履歴でメタフィールドをページネーション（first/after）で分割取得し、大量データでも取得を安定化。③**設定画面の表示件数説明**：各表示件数項目の説明を「最大〇〇・推奨〇〇」のみに簡略化。④**棚卸POS**：stocktakeApi に readInventoryCountsFirstPage・readInventoryCountsPage とヘルパーを追加。InventoryCountConditions を分割取得（ページネーション）対応に変更。実装: app.settings.tsx、各履歴 loader/API、extensions/stock-transfer-stocktake（stocktakeApi.js、InventoryCountConditions.jsx）。 | ✅ 完了 |
| **アプリタイル一覧のロケーション・表示件数・タップ後詳細の統一（2026-02-25）** | ①**ロケーション**：発注・仕入・ロス・調整でロケーションIDをGID/数値で正規化して比較（normalizeLocationGidForCompare）し、ログイン中ロケーションのみ表示。棚卸はフィルタ後0件でも全件フォールバックを削除。②**並び順**：全5タイルで履歴一覧を日付降順で表示。③**タップ後詳細**：発注・ロス・調整・仕入は一覧stateを軽く（stripEntryForListでitemsを落とす）、フルエントリはfullEntriesByIdRefに保持し、タップ時にrefから詳細表示（入庫・出庫と同様の操作感で一覧フリーズ短縮）。実装: OrderHistoryList.jsx, LossHistoryList.jsx, AdjustmentHistoryList.jsx, PurchaseHistoryList.jsx, InventoryCountConditions.jsx。docs/APP_TILE_LIST_LOAD_AND_LOCATION_FIX.md。 | ✅ 完了 |
| **棚卸：商品リスト「さらに読み込む」・下書き復元・まとめて表示読込・在庫数読込（2026-02-25）** | ①**コレクション経路のページネーション**：fetchProductsByGroups で collection.products に pageInfo/after を追加し、250件超を取得可能に。クライアントで collectionPageInfoRef を保持して次回渡す。②**下書き復元時**：hasMoreProducts を再設定（inventoryItemIdsByGroup の総数と復元行数比較、または initialLimit 以上で true）し、未読込リストを復元後も読めるように。③**まとめて表示の読込**：loadGroupProducts 内の c→count 参照ミスを修正（Can't find variable c 解消）。④**商品グループ一覧の在庫数読込**：全グループ並列・全商品一括 getCurrentQuantity だとレート制限で0件になるため、グループを順次処理・getCurrentQuantity を15件ずつバッチ化。実装: stocktakeApi.js, InventoryCountList.jsx, InventoryCountProductGroupSelection.jsx。docs/STOCKTAKE_UX_TAP_AND_LOAD_BUTTONS.md。 | ✅ 完了 |
| **履歴詳細の商品リスト：初回表示件数・さらに読み込む（ロス・注文・調整・発注）（2026-02-25）** | 各履歴一覧で詳細表示時の商品リストに、設定の productList.initialLimit（既定250）を反映。表示は entry.items を slice(0, detailDisplayLimit) で表示し、超過分は「さらに読み込む」で detailDisplayLimit を600件ずつ増加。実装: LossHistoryList.jsx, OrderHistoryList.jsx, AdjustmentHistoryList.jsx, PurchaseHistoryList.jsx。docs/STOCKTAKE_UX_TAP_AND_LOAD_BUTTONS.md。 | ✅ 完了 |
| **棚卸・全タイル：追加読み込みタイムアウト延長・読み込みボタン即時フィードバック（2026-02-25）** | ①**棚卸**：fetchProductsByGroups のタイムアウトを初回40秒・追加60秒に変更（初回20秒の根拠なし）。②**全タイル**：読み込み系ボタンで setLoading(true) 等の直後に await setTimeout(0) を挿入し、押した直後に「読込中...」を描画してからAPI取得開始。対象：注文・ロス・調整・発注・入庫・出庫・棚卸の履歴/商品リスト/検索/在庫更新等。実装: stocktakeApi.js, 各 *HistoryList.jsx, *ProductList.jsx, ModalOutbound.jsx, InboundListScreen.jsx, Modal.jsx, InboundShipmentSelection.jsx, InventoryCountConditions.jsx, InventoryCountProductGroupSelection.jsx, OutboundReadyToShipEdit.jsx。 | ✅ 完了 |
| **棚卸：管理画面の商品リストモーダルで編集ボタン押下後すぐに状態が戻る不具合（2026-02-25）** | 要因：モーダル表示中の useEffect が modalCount?.id 等の変化で再実行され、setModalEditMode(false) 等で編集状態がリセットされていた。対応：編集モード・編集数量のリセットを「モーダルを閉じたとき」のみに限定（useEffect の分岐を変更）。実装: app.inventory-count.tsx。 | ✅ 完了 |
| **棚卸：管理画面でモーダルを閉じて再度開くと読み込まれない商品グループが発生する不具合（2026-02-25）** | 要因：useFetcher の data はアンマウント後も残るため、モーダル再オープン時に前回のレスポンスが処理され、incompleteGroupFetchIndexRef 等がずれて一部グループが読まれない状態になっていた。対応：lastSubmittedForModalRef で「今回のモーダルで submit した countId/groupId」を保持し、fetcher.data はその組み合わせと一致するときのみ反映。実装: app.inventory-count.tsx。 | ✅ 完了 |
| **棚卸：同一ショップで複数ブラウザが重い棚卸モーダルを同時に開いた際の負荷軽減（2026-02-25）** | get_incomplete_group_products のレスポンスをサーバー側で in-memory キャッシュ。キー: shop:countId:groupId:offset、TTL 2分。棚卸の数量更新・確定・キャンセル・リセット等の action で当該棚卸のキャッシュを無効化（invalidateIncompleteGroupProductsCacheForCount）。同一ショップ複数ブラウザで同じグループを開いた場合に Shopify API 呼び出しを削減。docs/ADMIN_INVENTORY_COUNT_MULTI_BROWSER_LOAD.md。実装: app.inventory-count.tsx。 | ✅ 完了 |
| **棚卸：POSアプリタイルでまとめて表示・グループごと表示時に完了/キャンセル済みグループが下書きで上書きされる不具合（2026-02-25）** | 完了・キャンセル済みグループは常に API の groupItems を表示し、未完了グループのみ下書きを参照。履歴から完了/キャンセル棚卸を開いた場合も下書き復元せず API データを表示。実装: InventoryCountList.jsx。docs/STOCKTAKE_DRAFT_AND_READONLY_QA.md。 | ✅ 完了 |
| **棚卸：ドキュメント追加（2026-02-25）** | ①**ADMIN_INVENTORY_COUNT_MULTI_BROWSER_LOAD.md**：同一ショップで複数ブラウザが重い棚卸モーダルを同時に開いた際の負荷要因（Node 並行・Shopify API レート制限）とサーバーキャッシュ対策、同一ショップと複数ショップの違い。②**ADMIN_STOCKTAKE_MODAL_LOAD_AND_CONFIRM_CANCEL.md**：商品リスト読み込み中に一括確定・一括キャンセルした場合の挙動（一括確定は読込中は無効、一括キャンセルは有効・読込中リクエストは完了する可能性あり）。③**STOCKTAKE_5SEC_SPEC_ESTIMATE.md**：約5秒で処理完了する目安（リスト表示のみ／リスト＋在庫数表示）、グループ数が効くのはまとめて表示のみ。 | ✅ 完了 |
...
**審査まわり（対応済み）**
- **必須コンプライアンスWebhook**：実装済み（customers/data_request, customers/redact, shop/redact の購読と `/webhooks/compliance` エンドポイント・HMAC 検証）。パートナーダッシュボードの「一般的なエラーの自動チェック」は **通過済み**。提出前にデプロイ（`shopify app deploy` とホスティング側のデプロイ）を実施しておくこと。
...
| 3 | **Cron の設定**：在庫高の日次スナップショット用に `/api/inventory-snapshot-daily` を毎日（例: 23:59）呼ぶ。`INVENTORY_SNAPSHOT_API_KEY` を**Cron Job と Web サービスの両方**の環境変数に**同じ値**で設定（Web サービス側に無いと API が 401 を返す）。 | docs/CRON_JOB_SETUP_GUIDE.md | **推奨**（設定しないと前日分は管理画面を開いたときのフォールバックのみ） |
...
**まとめ**: コード・機能はリリース可能な状態。あとは **①本番DB ②初回オープン案内 ③Cron（推奨）** の運用と、**④公開用環境 ⑤リスティング ⑥審査提出** で公開アプリ完了まで進められる。在庫変動のアクティビティ反映・二重防止・「管理は管理画面のみ」の保証設計は **`docs/INVENTORY_ACTIVITY_REFLECTION_GUARANTEE.md`** にまとめている（公開アプリ販売前の確認用チェックリスト含む）。
...
| **微調整済み** | 棚卸ページ：上部メニュー下の余白を狭く（タブ＋区切り線を1ブロックにまとめて余白削減）。設定ページ：「破棄」「保存」は下部固定フッターに常時表示し、左にステータス（保存しました/未保存の変更があります等）を表示 |

**直近の主な更新（2026-02）**:
- **棚卸・全タイル：追加読み込みタイムアウト延長・読み込みボタン即時フィードバック（2026-02-25）**：①**棚卸**：fetchProductsByGroups のタイムアウトを初回40秒・追加読み込み60秒に変更（初回20秒に根拠がなかったため）。②**全タイル**：読み込み系ボタンで setLoading(true) 等の直後に `await new Promise(r => setTimeout(r, 0))` を挿入し、押した直後に「読込中...」を描画してからAPI取得開始（押したか分からない問題を解消）。対象：注文・ロス・調整・発注・入庫・出庫・棚卸の履歴再読込/読込/さらに読み込む/検索/在庫更新等。実装: stocktakeApi.js, 各 *HistoryList.jsx, *ProductList.jsx, ModalOutbound.jsx, InboundListScreen.jsx, Modal.jsx, InboundShipmentSelection.jsx, InventoryCountConditions.jsx, InventoryCountProductGroupSelection.jsx, OutboundReadyToShipEdit.jsx。
- **アプリタイル一覧のロケーション・表示件数・タップ後詳細の統一（2026-02-25）**：①**ロケーション**：発注・仕入・ロス・調整でロケーションIDをGID/数値で正規化してフィルタ（他ロケーション非表示）。棚卸はフィルタ後0件でも全件フォールバックを削除。②**並び順**：全5タイルで履歴一覧を日付降順で表示。③**タップ後詳細**：発注・ロス・調整・仕入は一覧を軽く（itemsを落としたstate）、詳細はタップ時にrefから取得し入庫・出庫と同様の操作感に統一（一覧フリーズ短縮）。実装: OrderHistoryList.jsx, LossHistoryList.jsx, AdjustmentHistoryList.jsx, PurchaseHistoryList.jsx, InventoryCountConditions.jsx。docs/APP_TILE_LIST_LOAD_AND_LOCATION_FIX.md。
- **アプリ表示件数統一・メタフィールド分割取得・設定説明・棚卸POS（2026-02-24）**：①**アプリ表示件数統一**：履歴一覧・商品リスト・検索リストの表示件数を設定で統一。②**メタフィールド分割取得**：発注・仕入・ロス・調整・棚卸の履歴でメタフィールドを first/after で分割取得し大量データでも安定化。③**設定画面**：表示件数項目の説明を「最大・推奨」のみに変更。④**棚卸POS**：stocktakeApi に readInventoryCountsFirstPage/Page とヘルパー追加、InventoryCountConditions を分割取得対応に変更。実装: app.settings.tsx、各履歴、extensions/stock-transfer-stocktake（stocktakeApi.js、InventoryCountConditions.jsx）。
- **棚卸：商品リスト読み込み安定化・完了/処理中不整合・数量0/-・ステータス統一・グループ削除後完了（2026-02-24）**：①**管理画面**：未完了グループの商品取得でキー正規化（先頭グループの「まだ処理されていません」照合漏れ防止）・取得中は「読み込み中...」・API エラー時も loading 解除・get_incomplete_group_products で locationId を GID 正規化（一覧・モーダルの数量表示改善）・一覧の incompleteGroupProductsForList をキー正規化（母数「/-」防止）・完了判定で「商品グループ設定で削除されたグループ」は完了とみなす。②**POS**：在庫調整なし・在庫調整ありの両方で「全て0・差分なし」のグループも groupItems に保存（管理画面とアプリの完了表示を一致）。readInventoryCounts で後方互換を廃止し完了は allDone のみに統一、readProductGroups で削除済みグループを完了扱いし必要時はメタに書き戻し。要因・対策は docs/STOCKTAKE_ADMIN_VS_APP_STATUS_CAUSE.md。実装: app.inventory-count.tsx、InventoryCountList.jsx、stocktakeApi.js。
- **ホーム・設定・料金まわり（2026-02-22）**：①**ホーム・スマホ**：767px以下で現在の料金プラン・ロケーション数を最上部に100%幅で表示（order/flex で制御）。②**ホーム・POSステップ**：「POSの設定を開く」で admin.shopify.com/store/{handle}/apps/point-of-sale-channel を、「販売チャネル一覧を開く」で settings/channels を新しいタブで開く。loader で storeHandle を取得し Outlet context に渡す。③**設定画面**：破棄・保存ボタンを画面下部に固定フッターで常時表示。左にステータス（保存しました/未保存の変更があります/保存中...等）、右に破棄・保存ボタン。④**パートナー価格**：docs/PARTNER_PRICING_SETUP_STEPS.md を追加。4プラン（lite-3, lite-10, pro-3, pro-10）の設定手順・ハイフン利用・表示名に Lite/Pro と 3/10 を含める前提を記載。⑤**ロケーション数とプラン不一致**：サブスク名から getMaxLocationsFromSubscriptionName で3/10を判定。実際のロケーション数がプランを超える場合は locationPlanMismatch を立て、ホーム・料金プラン以外へのアクセスを /app/plan にリダイレクト。画面上部に案内バナー、料金プラン画面で案内ブロックと「Shopifyでプランを変更する」リンク。⑥**Pro料金**：10ロケーションまで $120/月、11ロケーション目以降 $12/ロケーションに変更（billing.ts・app.plan.tsx・関連ドキュメント）。実装: app._index.tsx, app.tsx, app.settings.tsx, app.plan.tsx, app/utils/billing.ts, docs/PARTNER_PRICING_SETUP_STEPS.md。
- **公開アプリ課金実装（2026-02）**：①**プラン取得**：getShopPlan で currentAppInstallation.activeSubscriptions を取得し、サブスク名から lite/pro を判定。開発ストアは課金なしで plan=pro。②**Billing ユーティリティ**（app/utils/billing.ts）：getPlanFromActiveSubscriptions・getMaxLocationsFromSubscriptionName・reportUsageRecord・createAppSubscription（appSubscriptionCreate：Recurring＋Usage キャップ。Lite $40/7日 trial、Pro $120/14日 trial、Pro 従量 $12/loc）。③**10ロケーション超**：公開・本番・Lite/Pro・locationsCount>10 のとき idempotencyKey 付きで Usage 報告を fire-and-forget。④**料金プランページ**：「このプランを選択する」を form POST→createAppSubscription→confirmationUrl へ redirect。設計: docs/PUBLIC_APP_PLAN_FEATURES_DESIGN.md セクション8。
- **POS出庫確定後処理の他タイル統一・発注希望納品日UI（2026-02-21）**：①**出庫確定後**：確定後にコンディション画面に戻らない・下書きが削除されない事象を解消。他タイル（ロス・発注等）と同様に「確定処理の成功後に必ず下書き削除→モーダル閉じる→画面遷移」が実行されるよう統一。`logInventoryChangeToApi` を try/catch で囲み API 失敗時も後処理（下書き削除・コンディション遷移）を確実に実行。遷移は `submitTransferCore` 内ではなく確定ボタンの呼び出し側で、モーダルを閉じたあとに `onConfirmSuccess` を1回だけ実行。②**発注・希望納品日**：入力欄と日付クイックボタンの間隔が無く右寄せもされていなかったため、出庫の「到着予定日」と同様に `gap="small"`・`justifyContent="end"`・`wrap` を指定して間隔と右寄せを統一。実装: `extensions/stock-transfer-tile/src/ModalOutbound.jsx`、`extensions/stock-transfer-order/src/screens/order/OrderConditions.jsx`。
- **管理画面・調整の改善・在庫変動履歴の返品フィルター統一（2026-02-20）**：①**調整設定CSVに「差分」追加**：調整履歴CSV出力項目に delta（差分＝実数−在庫数）を追加。一括・モーダルCSVの両方で出力。②**調整履歴を「メモ」表示に統一**：一覧リストと商品リストモーダルの情報欄を「理由」→「メモ」に変更。③**調整の商品リストモーダルをCSV設定に連動**：モーダル内テーブルの列を設定の並び・ON/OFFに合わせて表示（明細列＋差分）。④**在庫変動履歴のアクティビティフィルター**：返品が2種（refund・order_cancel）あったのを、仕入と同様に1つの「返品」に統一。選択時は両方を検索対象に展開、URL正規化で旧URLも対応。実装: `app.adjustment.tsx`、`app.settings.tsx`、`app.inventory-info.tsx`。
- **簡易棚卸（調整）機能・調整設定・変動履歴「調整」・商品リストフッター（2026-02-20）**：①**簡易棚卸（調整）**：POS拡張 `stock-transfer-adjustment` を追加（ロス拡張をベースに複製）。コンディションはロケーション・日付・スタッフ・メモのみ（理由なし）。商品リストは在庫数＋実数で、確定時に `adjustInventoryToActual` と `logInventoryChangeToApi(activity: "adjustment")` で変動履歴に記録。履歴は #A0001 形式、キャンセル時は在庫を元の currentQuantity に戻す。②**管理画面**：設定の上タブに「調整設定」を追加し「調整履歴CSV出力項目設定」を配置（棚卸と同UI）。メニューに「調整」を棚卸の直下に配置。`app.adjustment.tsx` で調整履歴一覧・フィルター・商品リストモーダル・CSV出力（商品グループ列なし）。③**変動履歴**：アクティビティに「調整」を追加（`activity: adjustment`）。`inventory-change-log.ts`・`api.log-inventory-change`・`app.inventory-info.tsx` のラベル・フィルターに対応。④**調整タイル**：説明から「（簡易棚卸）」を削除。⑤**デプロイ**：調整拡張の UID をロスと重複しない一意の値に変更（Duplicate UID エラー解消）。発注 OrderConditions の `!!x ?? true` を `!!(x ?? true)` に修正（suspicious-nullish-coalescing 警告解消）。⑥**商品リストフッター**：棚卸の固定フッターに「明細 N」を追加。全商品リスト（入庫・出庫・仕入・発注・ロス・棚卸・調整）でフッター中央に明細を含む表示に統一し、2行以上のときは中央揃え。調整はスキャン・検索で商品追加時は実数=1で追加し、数量ボタンでカウント可能に。実装: `extensions/stock-transfer-adjustment`、`app/routes/app.adjustment.tsx`、`app.settings.tsx`、`app.tsx`、`app.inventory-info.tsx`、`api.log-inventory-change.tsx`、`app/utils/inventory-change-log.ts`、各拡張の FixedFooterNavBar・商品リストフッター、`docs/SIMPLE_STOCKTAKE_ADJUSTMENT_FEASIBILITY.md`。
- **入庫メモ1回追記・予定外表示フォールバック（2026-02-20）**：①**入庫メモを1回追記に変更**：これまで「1回目：予定外ブロック」「2回目：在庫調整履歴ブロック」の2回追記していたが、2回目の失敗で在庫調整履歴がメモに残らない事象を防ぐため、在庫の受入・調整をすべて実行したあと、**「予定外＋予定超過＋メモ＋在庫調整履歴」を1本にまとめて1回だけ**追記するよう変更。②**予定外形式B(旧)のオプション表示フォールバック**：メモが形式B(旧)（行に「オプション:」が無く、タイトルに「 / 」でオプションが含まれる）の場合、管理画面でタイトルを「商品名」と「オプション」に分離して表示。過去の確定分もオプション列に表示可能。③**要因ドキュメント**：`docs/INBOUND_NOTE_FORMATS_AND_CAUSES.md` にメモ形式差・在庫調整履歴の有無・二重記載の要因を追記。実装: InboundListScreen.jsx、app.history.tsx、INBOUND_NOTE_FORMATS_AND_CAUSES.md。
...
- **棚卸：まとめて表示の安定化（2026-02-17）**：①まとめて表示のループ前に readProductGroups() を1回だけ実行し、cachedProductGroups として全 fetchProductsByGroups に渡す（全グループが同じスナップショットを参照して初回から安定表示）。②stocktakeApi の fetchProductsByGroups に cachedProductGroups オプションを追加。③未完了グループで商品0件の場合、約1秒待機して1回だけリトライ。④要因ドキュメントに「グループごとで開いたあとだと表示される理由」「まとめて表示で不備なく表示するためのコード改善」を追記。実装: InventoryCountList.jsx, stocktakeApi.js, docs/STOCKTAKE_MULTIPLE_GROUPS_PRODUCT_LIST_CAUSE.md。
- **棚卸：CSVグループで履歴の数量母数・商品リスト表示対応（2026-02-17）**：CSVから作成したグループで、管理画面・アプリタイルの履歴一覧に数量母数が出ない・モーダルで商品リストが出ない事象に対応。get_incomplete_group_products にパターン1bを追加（collectionIds も inventoryItemIds も無いが skus がある場合、resolveSkusToInventoryItemIds で ID 解決してから商品・在庫を取得）。商品検索・コレクションから作成した場合は同様の要因にならない。要因・対策は `docs/STOCKTAKE_CSV_GROUP_HISTORY_QUANTITY_CAUSE.md`。実装: `app.inventory-count.tsx`。
- **在庫変動履歴：OrderPendingLocation/RefundPendingLocation の locationId="" 検索対応・残存リスク整理（2026-02-17）**：オンライン受注直後に「管理」のまま反映されない事象の要因として、OrderPendingLocation が `locationId=""` で登録されているのに検索で `.filter(Boolean)` により `""` が除かれマッチしなかったことを特定。orderLocCands から `.filter(Boolean)` を削除し `locationId=""` を検索に含めるよう修正（3箇所）。RefundPendingLocation は locCands に `""` を追加し、マッチ時に locationId を保持して deleteMany で使用するよう統一。要因: `docs/INVENTORY_ACTIVITY_MANAGEMENT_CAUSE_20260217.md`。残存リスク一覧: `docs/INVENTORY_REFLECTION_REMAINING_RISKS.md`。実装: `webhooks.inventory_levels.update.tsx`。
- **棚卸：まとめて表示の不具合修正・発行時全グループ取得・502対策（2026-02-16）**：①**0/0表示削除**：完了済み時の謎の「0/0」を削除し、件数0のときは「実数 X/Y」のみ表示。②**2つ目以降の商品リスト**：発行時に全選択グループの inventoryItemIds を取得（getInventoryItemIdsForGroup：既存がなければコレクション/SKUから取得）して inventoryItemIdsByGroup に保存。POS 側で ID 正規化・キー照合を追加し、まとめて表示で全グループの商品リストが読めるようにする。③**502対策**：発行時の商品ID取得に22秒の時間予算を設け、グループごとに try/catch して platform の30秒タイムアウトを避ける。要因・対策は `docs/STOCKTAKE_MULTIPLE_GROUPS_PRODUCT_LIST_CAUSE.md`。実装: `app.inventory-count.tsx`、`extensions/stock-transfer-stocktake`（InventoryCountList.jsx, stocktakeApi.js）。
- **在庫変動履歴：OrderPendingLocation に locationId・fulfillments ループで履行ごと登録（2026-02-16）**：同一注文で同一商品が複数ロケーションから出荷されるケースに対応。①OrderPendingLocation に `locationId` を追加（ユニーク・インデックス変更、マイグレーション 20260216000000）。②orders/updated の fulfillments あり時、全履行をループして各履行の location_id と line_items（variant_id, quantity）を収集し、(item, location) 単位で quantity を集約。③各 (item, location) で既存 order_sales がなければ admin_webhook を order_sales に救済、または OrderPendingLocation を upsert（locationId 付き）。処理前にこの注文の OrderPendingLocation を一括削除してから今回分のみ登録。inventory_levels/update のマッチ・削除にも locationId を追加。実装: `webhooks.orders.updated.tsx`、`webhooks.inventory_levels.update.tsx`、`prisma/schema.prisma`。
- **出庫商品リストの配送情報モーダル：arrivalQuickDays 未定義エラー修正（2026-02-15）**：出庫の商品リスト画面で「配送情報」を開くと「Can't find variable: arrivalQuickDays」が発生する不具合を修正。OutboundList 内に arrivalQuickDays の useMemo を追加。実装: `extensions/stock-transfer-tile/src/ModalOutbound.jsx`。
...
- **在庫高の本日集計の自動保存と保存時間表示（2026-02-11）**: ①**自動保存機能**: 在庫高画面で今日の日付を選択した際、スナップショットが存在しない場合は自動的にリアルタイムで在庫情報を取得してスナップショットを保存。初回表示時にバックグラウンドで保存処理を実行し、エラーが発生しても表示は続行。②**保存時間の表示**: スナップショットに`updatedAt`フィールドを追加し、保存時に現在時刻（ISO文字列）を設定。テーブルの最下部に「保存時間：YYYY/MM/DD HH:MM」形式で表示（今日のスナップショットが存在する場合のみ）。ショップのタイムゾーンに基づいて表示。③**Cron実行時の更新**: 夜のCronで実行された際も`updatedAt`が更新されるように実装。23時の実行時は今日のスナップショットを更新し、0時の実行時は前日のスナップショットを保存。実装: `app/utils/inventory-snapshot.ts`（`DailyInventorySnapshot`型に`updatedAt`追加、`saveSnapshotsForDate`で`updatedAt`設定）、`app/routes/app.inventory-info.tsx`（loaderで自動保存処理、actionで「本日集計」ボタン処理、UIで保存時間表示）。
- **在庫変動履歴のエラーハンドリング改善（2026-02-11）**: ①**タイムアウト対策**: 在庫変動履歴の取得処理（データベースクエリ）に30秒のタイムアウトを追加。`Promise.race`を使用してタイムアウトを実装し、タイムアウト時はエラーログを出力して処理を続行。②**エラーハンドリングの改善**: loader全体のエラーハンドリングを改善し、エラー発生時もJSONレスポンスを返すように変更。エラー発生時は空データを返して画面が表示されるように修正（`responseBytes=29`の問題を解決）。③**エラーログの詳細化**: エラーメッセージとスタックトレースをログに記録。タイムアウトエラーの場合は特別なログを出力。実装: `app/routes/app.inventory-info.tsx`（データベースクエリにタイムアウト処理、loader全体のエラーハンドリング改善、エラーログの詳細化）。
- **在庫変動履歴の二重記録防止とオンラインストア対応の改善（2026-02-11）**: ①**検索ロジックの改善**: `inventory_levels/update` webhookの検索ロジックを改善。タイムウィンドウを10分→30分に拡大し、10分以内に見つからない場合は30分前まで再検索。`orders.updated`や`refunds/create`で保存されたログを確実に検出できるように改善。②**オンラインストア対応**: `orders.updated` webhookで`fulfillment.location_id`がnullの場合の処理を追加。GraphQL APIで注文のデフォルトロケーションを取得するように修正。オンラインストアでも`order_sales`ログが作成されるように改善。③**デバッグログの強化**: 検索条件と結果を詳細に出力。実際に存在するログも確認して出力し、問題の原因を特定しやすくする。④**コードの重複修正**: 重複していた`if (recentNonAdminLog)`ブロックを削除。実装: `app/routes/webhooks.inventory_levels.update.tsx`（検索ロジック改善、デバッグログ強化）、`app/routes/webhooks.orders.updated.tsx`（オンラインストア対応）。
- **在庫変動履歴の売上・返品判定・変動数・二重防止の改善（2026-02-11）**: ①**OrderPendingLocation**：受注直後は FulfillmentOrder.assignedLocation が null のため `orders/updated` でロケーションが取れず「管理」のままになる問題に対応。`OrderPendingLocation` テーブル（prisma/schema.prisma、マイグレーション `20260211000000_add_order_pending_location`）を追加。ロケーションが取れない場合は line_items を一時保存し、`inventory_levels/update` で同一商品・時刻が近い保留注文と突き合わせて `order_sales` で保存。②**救済**：`inventory_levels/update` が先に届いて admin_webhook で保存された場合、後から届いた `orders/updated` で直近の admin_webhook を検索し `order_sales` に上書き（ロケーションなし時）。③**FLOW 等の別イベント**：既存ログの quantityAfter と今回の available が異なる場合は別イベントとみなし新規「管理」行を作成。④**POS 二重防止**：`api/log-inventory-change` の recentTo を「ts+5分」と「現在+2分」の遅い方に拡大。既存の order_sales/refund が同一 item・location・時間にいる場合は新規作成せず更新。⑤**返品**：`refunds/create` の searchTo を「返品作成+5分」と「現在+2分」の遅い方に拡大。⑥**変動数**：売上はオーダー数量を変動数に反映。`inventory_levels/update` で OrderPendingLocation マッチ時は `delta = -pendingOrder.quantity`。`orders/updated` で order_sales に更新する際は `delta = -(lineItem.quantity)` で統一。実装: `webhooks.inventory_levels.update.tsx`、`webhooks.orders.updated.tsx`、`webhooks.refunds.create.tsx`、`api.log-inventory-change.tsx`。
- **在庫変動履歴の売上・返品：受注時優先・到着順非依存・POS救済（2026-02-12）**: ①**受注時（fulfillments=0）**で常に OrderPendingLocation を登録（ロケーションの有無にかかわらず）。後から届く inventory_levels/update がマッチして order_sales＋変動数で記録。②**配送完了時（fulfillments あり）**は、この注文で order_sales が1件でも既にあれば救済をスキップ（オンラインは受注時で反映済みのため）。③fulfillments ありで admin_webhook がまだ無い場合（POS で orders/updated が先に届く等）は OrderPendingLocation に登録し、後から inventory_levels/update でマッチ。④admin_webhook を order_sales に更新した際は該当 OrderPendingLocation を削除して二重記録防止。⑤**販売可能数と手持ち在庫数は分けない**方針で運用。ドキュメント: `docs/SALES_REFUND_ACTIVITY_AND_WEBHOOK_API_FLOW.md`（届く情報の差・オンライン/POS の扱い・Webhook 到着順）、`docs/SALES_REFUND_VERIFICATION_CHECKLIST.md`（検証チェックリスト）。実装: `webhooks.orders.updated.tsx`。
- **在庫変動履歴のアクティビティ「管理」残り対策（2026-02-12）**: 履歴とRenderログで 18:29（複数明細の5本目だけ救済漏れ）・19:30（同一商品の連続売上2件目が「管理」）の要因を分析。`inventory_levels/update` で admin_webhook を保存する**直前に** OrderPendingLocation を再検索し、登録されていれば order_sales で保存するように変更。レース・連続売上で「管理」が残るケースを軽減。要因分析: `docs/INVENTORY_ACTIVITY_MANAGEMENT_CAUSE_20260212.md`。実装: `webhooks.inventory_levels.update.tsx`。
- **在庫高の日付・本日集計のスマホレイアウト（2026-02-12）**: 実機のアプリブラウザで日付枠が切れる・本日集計ボタンが改行される事象に対応。フィルター白カード・「日付＋本日集計」行・左カラムに `minWidth: 0` / `overflow: hidden` / `flexWrap: nowrap` / `maxWidth: 100%` を付与し、日付とボタンを同一行で表示しはみ出さないように修正。実装: `app.inventory-info.tsx`。
- **在庫変動履歴：既存 admin_webhook を order_sales に更新して二重「管理」防止（2026-02-12）**: OrderPendingLocation にマッチしたとき、新規行を作る前に同一 item/location で「quantityAfter = available + 売上数量」の admin_webhook が時間窓内にないか検索。見つかったらその既存行を order_sales に更新し新規行は作らない（20:11/20:14 型の同一商品で「管理」が2行並ぶ事象を防ぐ）。実装: `webhooks.inventory_levels.update.tsx`。
- **在庫変動履歴：注文編集時の救済で時間窓を updated_at 基準に変更（2026-02-12）**: fulfillments なし（注文編集等）時の救済で、時間窓が注文作成日時（created_at）基準だったため、昔に作成された注文を編集した場合に編集直後の admin_webhook が窓に入らず救済されない問題を修正。searchFrom/searchTo を注文更新日時（updated_at）基準に変更。実装: `webhooks.orders.updated.tsx`。
- **変動一覧下の注釈文言・在庫高の本日集計で全ロケーション表示（2026-02-12）**: ①変動一覧リスト下の注釈を「管理画面からの在庫数量変更：対象SKUとロケーションの変動が初回の場合は変動数が「-」表記になります。」に変更（改行＋「変動数が」を追記）。②本日を選択したときにロケーションが1つしか表示されない不具合を修正。原因は todaySnapshot を .find() で1件だけ使い currentInventory を1件で作っていたため。本日分も .filter(s.date === todayInShopTimezone) で全件取得するように変更。要因・検証は `docs/INVENTORY_INFO_TODAY_SNAPSHOT_ONE_LOCATION.md`。実装: `app.inventory-info.tsx`。
- **Cron 日次スナップショット・在庫変動履歴の改善（2026-02-13）**: ①**Cron 認証**：401 の要因として Web サービスの Environment に `INVENTORY_SNAPSHOT_API_KEY` が必須であることを明文化。②**Cron 最終確定**：既存のスナップショットがあっても上書きするよう変更（スキップ廃止）。③**Cron 並列処理**：複数ショップを同時処理（デフォルト 3、`SNAPSHOT_CONCURRENCY` で 1〜10）。④**在庫変動履歴ページネーション**：5000 件/ページ、前へ/次へ（入出庫履歴と同様 UI）。⑤**在庫変動履歴 CSV**：検索結果全件ダウンロード（最大 5 万件）。⑥**変動一覧注釈**：管理画面からの在庫数量変更の説明＋CSV ダウンロード制限の案内を追加。⑦**ACTIVITY_LABELS** 定数化。実装: `api.inventory-snapshot-daily.tsx`、`app.inventory-info.tsx`。ドキュメント: `docs/CRON_JOB_SETUP_GUIDE.md`。
- **在庫変動：アクティビティ反映の強固化・二重防止・キャンセル戻り（2026-02-13）**: ①**変動数「-」対策**：直前ログの quantityAfter が null のとき同一 item/location で遡って delta 算出。既知アクティビティ行の quantityAfter 更新時に delta が null なら補完。②**直近が admin_webhook のみ**のとき、2 分前〜1 分後の admin_webhook で quantityAfter 一致または null ならその行を更新して新規を作らない（二重防止）。③**knownActivities に purchase_cancel 追加**。④**注文キャンセル**：cancelled_at 時に在庫戻りを order_cancel で記録。既存 admin_webhook を order_cancel に更新して二重防止。⑤**api/log-inventory-change**：同一 activity（入庫・出庫・ロス・棚卸・仕入・仕入キャンセル）の既存行があれば更新して新規 create しない（POS 重複送信対策）。⑥**order_cancel** のラベル「キャンセル戻り」・フィルター・型を追加。実装: `webhooks.inventory_levels.update.tsx`、`webhooks.orders.updated.tsx`、`api.log-inventory-change.tsx`、`app.inventory-info.tsx`、`inventory-change-log.ts`。ドキュメント: `docs/INVENTORY_ACTIVITY_REFLECTION_GUARANTEE.md`、`docs/INVENTORY_POS_DELTA_AND_ACTIVITY_WEBHOOK_ANALYSIS.md`、`docs/INVENTORY_ACTIVITY_MANAGEMENT_CAUSE_20260213.md`。
- **在庫変動：反映漏れ洗い出し・強固化・大量処理チャンク（2026-02-13）**: ①**公式整理**：管理画面 Webhook に変動数が含まれないことを REST InventoryLevel 仕様で整理し、反映漏れ・二重になりうる箇所を全経路で洗い出し（`docs/INVENTORY_WEBHOOK_DELTA_OFFICIAL_AND_REFLECTION_HARDENING.md`）。②**orders/updated**：ロケーション不明時は他ロケーションの admin_webhook を誤更新しないよう救済をスキップ。③**大量在庫処理**：200SKU 以上の一括で API タイムアウトによる「管理」残りを防ぐため、POS の `logInventoryChange.js` を 50 件チャンク送信に変更。実装: `webhooks.orders.updated.tsx`、`extensions/common/logInventoryChange.js`。
- **在庫変動履歴UI：アクティビティラベル・文言改善（2026-02-13）**: ①**ラベル振り分け**：仕入キャンセル→「仕入」、キャンセル戻り→「返品」に表示統一。②**変動後在庫数**：一覧・CSV の「変動後数量」→「変動後在庫数」に文言変更。③**getActivityDisplayLabel**：大文字小文字・空白に強く、order_sales→売上・admin_webhook→管理が確実に表示されるよう堅牢化。未マッピング時は「その他」。④**チャンク送信リトライ**：`logInventoryChange.js` で失敗時に最大 2 回リトライ。⑤**初回管理画面案内**：README に追記。⑥**監視・確認チェック**：INVENTORY_ACTIVITY_REFLECTION_GUARANTEE に追記。⑦**反映漏れ対応状況**：`docs/INVENTORY_REFLECTION_STATUS_AND_REMEDIATION.md` を新規・更新。実装: `app.inventory-info.tsx`、`extensions/common/logInventoryChange.js`、README.md。
- **公開/自社デプロイ手順（2026-02-13）**: 公開アプリとカスタムアプリの2種運用のため、①手順ドキュメント `docs/DEPLOY_PUBLIC_AND_INHOUSE.md`（パートナーダッシュボード・CLI の注意と APP_MODE 切り替え）、②`scripts/set-app-mode.js`（APP_MODE の書き換え）、③npm スクリプト `deploy:public` / `deploy:inhouse`（切り替え＋config use＋deploy）を追加。実装: `package.json`、`scripts/set-app-mode.js`。
...
- **出庫「確定する」で常に処理中（2026-02-14）**: 出庫モーダルで「確定する」を押したときに、追跡情報の有無にかかわらず常に Shipment を IN_TRANSIT で作成するよう変更。以前は追跡ありの場合のみ「処理中」になり、追跡なしだと「配送準備完了」のままだった。実装: `ModalOutbound.jsx` の `submitTransferCore`。
- **在庫変動履歴：履歴を意図通りに扱うための修正（2026-02-16）**: ①救済時の update で idempotencyKey を変更しない（orders/updated・refunds/create・inventory_levels/update）。P2002 で救済失敗する残りを防止。②inventory_levels/update の create で P2002（idempotencyKey 重複）をキャッチして 200 を返す。③OrderPendingLocation/RefundPendingLocation マッチ時、quantityAfter === available の既存 admin_webhook も order_sales/refund に更新（二重行防止）。④OrderPendingLocation 待機・再検索を 2 回→3 回（約 7.5 秒）に増加。ドキュメント: `docs/INVENTORY_REFLECTION_STATUS_AND_REMEDIATION.md` に待機一覧・完全化の修正一覧を追加。
- **在庫変動履歴の二重防止：Webhook 時間窓60秒（2026-02-14）**: `inventory_levels/update` で既存の既知アクティビティ行（出庫・入庫等）の quantityAfter を更新する時間窓を 2 分から **60 秒**に短縮。新規「管理」行を作らず既存行を更新する二重防止の精度を向上。実装: `webhooks.inventory_levels.update.tsx`。
- **在庫高表示文言の変更（2026-02-14）**: ①Cron による過去日付スナップショットの説明文を削除。②スナップショット未存在時を「〇〇の在庫高データが見つかりませんでした。」に変更し、日付表示を YYYY/MM/DD に統一。実装: `app.inventory-info.tsx`。
- **在庫変動履歴で「仕入-」を「仕入」に表示統一（2026-02-14）**: 仕入キャンセル（purchase_cancel）の表示を「仕入-」から「仕入」に変更。実装: `app.inventory-info.tsx`。
- **ロス理由ボタンを右寄せに統一（2026-02-14）**: ロス登録の理由選択ボタンを、希望納品日・到着予定日ボタンと同様の右寄せ・折り返しレイアウトに変更。実装: `LossConditions.jsx`。
- **在庫変動履歴：CSV出力・オンライン売上完全反映（2026-02-14）**: ①**CSV出力**：在庫変動履歴の「CSV出力」ボタンで別タブに送る際、`form.action` に `location.search` を含めるよう修正。pathname のみだとショップ情報が渡らずドメイン入力画面になっていたため、同一URL（クエリ含む）で送信しスムーズにダウンロード可能に。実装: `app.inventory-info.tsx`。②**OrderPendingLocation 先登録**：orders/updated の fulfillments なし時、注文ロケーション取得（GraphQL）の**前**に line_items 分の OrderPendingLocation を登録するよう処理順を変更。inventory_levels/update が先に届いてもマッチしやすくする。実装: `webhooks.orders.updated.tsx`。③**待機＋再検索**：inventory_levels/update で「管理」で保存する直前に OrderPendingLocation が見つからない場合、2.5 秒待機してから最大 2 回再検索（計最大約 5 秒）。オンライン受注で inventory_levels/update が orders/updated より先に届いても、売上として確実に記録する。実装: `webhooks.inventory_levels.update.tsx`。ドキュメント: `docs/INVENTORY_REFLECTION_STATUS_AND_REMEDIATION.md`。
- **返品を売上と同様に処理：RefundPendingLocation・GraphQLフォールバック（2026-02-14）**: ①**RefundPendingLocation**：売上の OrderPendingLocation と同様に、返品用の一時テーブルを追加。refunds/create で返品ログ作成前に登録し、inventory_levels/update が先に届いた場合にマッチして「返品」で記録。②**line_item 検索失敗時のフォールバック**：order の lineItems で line_item_id が見つからない場合、GraphQL Refund API（`refund(id)` の `refundLineItems.edges[].node`）で quantity・location を突き合わせて inventory_item_id を取得。③**inventory_levels/update**：delta>0（在庫増）時に RefundPendingLocation を検索し、マッチすれば refund として記録。保存直前再検索・待機＋再検索・既存 admin_webhook→refund 更新で二重防止。④**キャンセル（order_cancel）**：既存実装を確認し、fulfillments と line_items からロケーション・バリアントが取得できており問題なしと確認。実装: `webhooks.refunds.create.tsx`、`webhooks.inventory_levels.update.tsx`、`prisma/schema.prisma`、マイグレーション `20260214000000_add_refund_pending_location`。
- **admin_webhook 未検出時リトライ 30 秒を全経路に適用（2026-02-15）**: 共通モジュール `admin-webhook-retry.ts` を作成し、売上・返品・入庫・出庫・ロス・棚卸・仕入の全アクティビティに 2.5 秒×12 回＝最大 30 秒の待機＋再検索を適用。①api/log-inventory-change、②webhooks.orders.updated（3箇所）、③webhooks.refunds.create。見つかり次第抜けて短い応答で返る。実装: `app/utils/admin-webhook-retry.ts`、`api.log-inventory-change.tsx`、`webhooks.orders.updated.tsx`、`webhooks.refunds.create.tsx`。ドキュメント: `docs/INVENTORY_REFLECTION_STATUS_AND_REMEDIATION.md`。
- **在庫変動履歴フィルターで「仕入」を1つに統一（2026-02-15）**: 仕入（purchase_entry）と仕入キャンセル（purchase_cancel）を同じ「仕入」として扱うため、フィルターの選択肢を「仕入」1つにまとめた。選択時は両方のアクティビティを対象に検索。実装: `app.inventory-info.tsx`。
...
- 売上・返品のアクティビティと Webhook/API の関係・受注時優先・到着順非依存: `docs/SALES_REFUND_ACTIVITY_AND_WEBHOOK_API_FLOW.md`
- 売上・返品まわり修正の検証チェックリスト: `docs/SALES_REFUND_VERIFICATION_CHECKLIST.md`
...
- **データ構造**:
  ...
  ```
- **UI**: 履歴一覧リスト・商品リスト・検索リストの**3つの数値入力**を別々に設定。半角数字以外（全角・スペース等）は検証し、入力欄下に「値を確認してください。半角数字で入力をお願いします。」を表示（2026-02追加）。履歴一覧リストは出庫履歴・入庫履歴の両方に同じ値で適用。各項目に「最大〇〇・推奨〇〇」の説明を表示。
- **デフォルト**: 
  - 履歴一覧リスト: 100件
  - 商品リスト: 250件
  - 検索リスト: 50件
...
5. 🔄 在庫情報機能の実装
   ...
   - ✅ **Phase 5-2: 在庫変動履歴** - 完了（2026-02-06完了）
     ...
     - ✅ **日付選択の制限**
       ...
       - ✅ 終了日の初期値：今日
     - ✅ **CSV出力機能**
       - ✅ 発生日時、SKU、ロケーション、アクティビティ、変動数、変動後在庫数、参照ID、備考を含むCSV出力（2026-02-13 に変動後数量→変動後在庫数に文言変更）
       - ✅ 日本語ヘッダー
       - ✅ BOM付きUTF-8形式
...
#### 出庫確定後の遷移・下書きリセット（2026-02-20）✅ 完了
- **現象**: アプリタイルで出庫確定後、商品リスト画面からコンディション画面に戻らず、モーダルを閉じて開き直すとコンディション・商品リストの自動保存内容が復元されてしまう。
- **要因**: ①確定後の `onBack()`（`nav.pop()`）のみでは、描画タイミング等で画面が切り替わらない場合がある。②コンディション下書きの `storage.delete` が失敗しても `catch (_) {}` で握りつぶしており、削除が反映されないとモーダル再オープン時に下書きが復元される。
- **修正内容**: ①**遷移**: 確定後は `onConfirmSuccess` で `nav.reset(SCREENS.OUTBOUND_COND)` を実行し、コンディション画面へ強制遷移。②**下書きクリア**: `OUTBOUND_CONDITIONS_DRAFT_KEY` は `storage.delete` に加え `storage.set(KEY, {})` で空オブジェクトを上書きし、delete 失敗時も次回オープンで復元されないようにした。新規確定・編集（同ID）確定・配送追加確定の全後処理で適用。
