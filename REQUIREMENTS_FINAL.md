...
| **変動履歴** | **api/log-inventory-change の admin_webhook 未検出時リトライを 4 回（最大 10 秒）に変更（2026-02-15）**：Webhook の create が API の検索より遅いレースで「管理」が残る漏れを減らすため、`ADMIN_WEBHOOK_RETRY_TIMES = 4`（2.5 秒×4 回＝最大 10 秒）に変更。見つかった時点でループを抜けるため、早く届けば 2.5 秒・5 秒など短い応答で返る。実装: `api.log-inventory-change.tsx`。ドキュメント: `docs/INVENTORY_REFLECTION_STATUS_AND_REMEDIATION.md`、`docs/WEBHOOK_LINKING_ISSUES_CAUSE.md`。 | ✅ 完了 |
...
- **api/log-inventory-change の admin_webhook リトライ 4 回・最大 10 秒（2026-02-15）**: Webhook の create が API より遅いレースで「管理」が残る漏れを減らすため、admin_webhook 未検出時のリトライを **4 回**（2.5 秒×4＝**最大 10 秒**）に変更。見つかった時点で抜けるため、早く届けば短い応答で返る。実装: `api.log-inventory-change.tsx`（`ADMIN_WEBHOOK_RETRY_TIMES = 4`）。ドキュメント: `docs/INVENTORY_REFLECTION_STATUS_AND_REMEDIATION.md`。
