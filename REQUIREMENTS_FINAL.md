...
| 棚卸ID採番の安定化・誤採番の自動修正 | ✅ 完了（2026-02-26。①POS：fixCountsStatusOnly で createdAt 昇順＋同順時 id でソートし #C0034 の後に作ったものが #C0028 になる事象を防止。②管理画面：create_inventory_count 時も同順でソートしてから次の連番を付与。③既に発行した誤った countName を次回読み込み時に作成順で再計算して修正（常に countName を再計算し anyCountNameChanged のとき保存）。実装: stocktakeApi.js, app.inventory-count.tsx） |
