...
| 棚卸：ロケーションに在庫レベルがない時の確定失敗を解消 | ✅ 完了（2026-02-27。在庫有効化処理を追加。adjustInventoryToActual 実行前に ensureInventoryActivatedAtLocation で nodes(ids) により在庫レベル未作成のアイテムを検出し、inventoryItemUpdate(tracked:true)・inventoryActivate でロケーションに在庫レベルを作成してから inventorySetQuantities を実行。出庫タイルの ensureInventoryActivatedAtLocation と同様の処理を stocktakeApi.js に実装。実装: extensions/stock-transfer-stocktake/src/screens/stocktake/stocktakeApi.js） |
