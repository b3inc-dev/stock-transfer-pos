...
| **棚卸** | ✅ 完了 | ID発行・未完了グループ商品取得（キー正規化・GID正規化・読み込み中表示）、まとめて表示安定化・CSVグループ対応、完了/処理中不整合解消・削除済みグループ完了扱い。POS: readInventoryCountsFirstPage/Page・InventoryCountConditions 分割取得。**2026-02-25** 商品リスト「さらに読み込む」をコレクション経路でも pageInfo/after で250件超取得可能に・下書き復元時 hasMoreProducts 再設定・まとめて表示読込の c→count 修正・商品グループ一覧の在庫数読込をグループ順次・15件バッチ化。app.inventory-count.tsx, extensions/stock-transfer-stocktake, docs/STOCKTAKE_UX_TAP_AND_LOAD_BUTTONS.md |
