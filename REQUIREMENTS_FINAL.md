...
| 棚卸：確定後のフッターステータス・差分あり時のバックグラウンド処理 | ✅ 完了（2026-02-27。①ステータス：差分あり・差分なしの全確定成功パスで onAfterConfirm(updatedCount) を渡すようにし、親で setCount(updatedCount) してフッターを「完了」に即時更新。②差分あり確定：adjustInventoryToActual を先に await してエラー要因を確認し、成功後に readInventoryCountsRaw で updated を組み立て→トースト・onAfterConfirm(updatedCount)・setSubmitting(false) でUIを先に完了→logInventoryCountToApi・writeInventoryCounts・clearAllInventoryCountDraftsForCount を Promise.all でバックグラウンド実行。まとめて表示・単一グループの両方で同様。実装: InventoryCountList.jsx, StocktakeScreen.jsx） |
...
| 棚卸：在庫差異なし時はモーダル即閉じ・確定後遷移でステータス即時反映 | ✅ 完了（2026-02-28。①在庫差異なし：InventoryCountList でまとめて表示・単一グループの2経路とも setTimeout(0) で組み立て・onAfterConfirm・read/write をバックグラウンドにし、即 return true でモーダルを閉じる（アプリタイル上で待たせない）。②確定後遷移：StocktakeScreen の handleAfterConfirm で setCount のあと、status===completed ならコンディション画面、未完了ありなら商品グループリストへ遷移。遷移先で親 state の更新済み count を表示するため、モーダルを閉じるまでステータスが完了にならない事象を解消。実装: InventoryCountList.jsx, StocktakeScreen.jsx） |
| 棚卸：アプリタイルリスト・確定処理高速化・完了表示・merge安全化 | ✅ 完了（2026-02-28。①アプリタイルからリスト表示時に棚卸IDが消えない：handleAfterConfirm(null) 時は何もしない・refresh 失敗時も setCounts([]) しない。②確定処理の高速化：buildUpdatedCountFromLocalState でローカル組み立て・read をブロックせず即完了表示・4経路で read をバックグラウンドに・readInventoryCountsFirstPage のチャンクを Promise.all で並列取得。③完了が未処理に戻る不具合：fixCountsStatusOnly/readInventoryCounts で status===completed は上書きしない。④確定後 merge・ID 比較の安全化：read に当該 count が無い場合は配列に追加・ID を String 比較に統一・Conditions のバックグラウンド詳細マージ・draft 更新で allCounts を Array ガード。実装: StocktakeScreen.jsx, InventoryCountConditions.jsx, InventoryCountList.jsx, InventoryCountProductGroupSelection.jsx, stocktakeApi.js） |
