...
| 棚卸：棚卸ID番号重複防止（新規作成時） | ✅ 完了（2026-02-28。管理画面の create_inventory_count で、保存直前に readInventoryCountsChunked で最新一覧を取得し、新規件は countName を空のまま fullList に追加。ensureCountNamesOnCounts(fullList) で付与してから writeInventoryCountsChunked に渡す。同時発行でも同一 max 参照による重複を防止。docs/STOCKTAKE_COUNTNAME_AUDIT.md に「番号重複の防止」を追記。実装: app.inventory-count.tsx） |
