...
| 在庫履歴のアクティビティ種別振り分け | ✅ 完了（管理/ロス/仕入/入出庫/棚卸/売上/返品/**キャンセル戻り（order_cancel）**。Webhook二重防止・api/log-inventory-change で種別記録・上書き。2026-02-10 に出庫確定・入庫通常受領でのAPI呼び出しを追加し全確定処理で網羅。2026-02-13 にアクティビティ反映の強固化・二重防止・変動数「-」対策を追加） |
